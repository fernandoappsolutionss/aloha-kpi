...
export default function EncuestasPanel({centroId,anio,mes,compact=false,onResumen,canWrite=true}) {
  ...
  return <section className="survey-panel" data-tour="encuestas.panel" aria-label="Encuesta de satisfacción mensual">
    ...
    <div className="survey-stats" data-tour="encuestas.progreso">
      ...
      <div className="survey-box"><span className="label">META DEL MES</span><strong>{r.necesarias||'—'} <small>respuestas</small></strong><p>{r.metaTexto} de los activos. {r.faltan>0?`Falta${r.faltan===1?'':'n'} ${r.faltan}.`:r.activos?'Participación alcanzada.':'Revisa el padrón activo.'}</p></div>
    ...
    <div className="survey-box" data-tour="encuestas.compartir">
      ...
      <p>{canWrite?'La copia del enlace o descarga del QR queda registrada.':'Modo consulta: no se copian enlaces ni se registra difusión desde esta cuenta.'} Para marcar el mes también se necesita {r.metaTexto.toLowerCase()} de respuestas de los niños del corte.</p>
