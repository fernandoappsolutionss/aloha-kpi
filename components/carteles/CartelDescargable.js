...
export default function CartelDescargable({ tipo, centroId, centro, canWrite = true }) {
  ...
  return <details className="poster-download" data-tour={`carteles.${tipo}`}>
    <summary>{canWrite ? 'Preparar cartel PDF' : 'Cartel PDF'}</summary>
    <div className="poster-download__body">
      <h4>{titulo} · {centro}</h4>
      <p>{canWrite ? 'Una hoja con diseño ALOHA, lista para imprimir y colocar en tu centro.' : 'Modo consulta: este material se prepara desde una cuenta operativa autorizada.'}</p>
      {tipo==='google'?<>
        <label>Enlace de reseñas de Google<input type="url" value={enlace} maxLength={2000} disabled={busy||!canWrite} onChange={e=>{setEnlace(e.target.value);setStatus('');setError('')}} placeholder="https://g.page/r/…/review"/></label>
        ...
        <label className="poster-download__check"><input type="checkbox" checked={detallesWifi} disabled={busy||!canWrite} onChange={e=>setDetallesWifi(e.target.checked)}/> Incluir red de invitados, clave y QR de conexión</label>
        {detallesWifi&&<>
          <label>Nombre exacto de la red de invitados<input autoComplete="off" value={red} maxLength={32} disabled={busy||!canWrite} onChange={e=>setRed(e.target.value)}/></label>
          <label>Clave de invitados (vacía si la red es abierta)<input type="text" autoComplete="off" value={clave} maxLength={63} disabled={busy||!canWrite} onChange={e=>setClave(e.target.value)}/></label>
      ...
      <label>Tamaño del papel<select value={formato} disabled={busy||!canWrite} onChange={e=>setFormato(e.target.value)}><option value="carta">Carta (21,6 × 27,9 cm)</option><option value="a4">A4 (21 × 29,7 cm)</option></select></label>
      {canWrite&&<button type="button" className="btn" disabled={busy||!centroId||!centro||centro==='Centro'||(tipo==='google'&&!valido)||(tipo==='wifi'&&detallesWifi&&!red)} onClick={descargar}>{busy?'Preparando PDF…':`Descargar PDF de ${titulo}`}</button>}
      <p>{canWrite?'Descargar prepara el material. Marca Sí cuando esté colocado y comprobado.':'Puedes revisar el criterio; la descarga queda deshabilitada en solo lectura.'}</p>
