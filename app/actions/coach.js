...
// del centro — esa pasa por requireCurrentCentroAccess como el resto del producto.
...
// propio centro, el global de centros registrados y el periodo. Los 4 candados contra el ruido y las frases viven
...
export async function getDesercionPorCoach(centroId, anio, trimestre) {
  await requireCurrentCentroAccess(centroId)
  const id = Number(centroId), year = Number(anio), quarter = Number(trimestre)
  if (!Number.isInteger(id) || id <= 0 || !Number.isInteger(year) || year < 2000 || year > 2100 || ![1, 2, 3, 4].includes(quarter)) {
