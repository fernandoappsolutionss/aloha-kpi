...
test('crear coach desde la ficha toma el nombre de la ficha y la enlaza', async () => {
  const fx = writeFixture({ actor: { id: 1, rol: 'admin_general', centros: [], password_hash: 'x' } })
  const result = await fx.service.create({ uid: 1 }, {
...
test('crear coach sin ficha sigue aceptando el nombre tecleado', async () => {
  const fx = writeFixture({ actor: { id: 1, rol: 'admin_general', centros: [], password_hash: 'x' } })
  await fx.service.create({ uid: 1 }, { nombre: 'Nueva Coach', email: 'n@aloha.invalid', rol: 'coach', centro_id: 10 })
...
test('la ficha del coach se valida: centro, estado y cuenta previa', async () => {
  ...
  for (const [coachId, mensaje] of casos) {
    const fx = writeFixture({ actor })
    await assert.rejects(
      () => fx.service.create({ uid: 1 }, { email: `c${coachId}@aloha.invalid`, rol: 'coach', centro_id: 10, coach_id: coachId }),
      new RegExp(mensaje.replace(/[.]/g, '\\.')),
    )
    assert.deepEqual(fx.inserted, [], `${coachId}: no se crea la cuenta`)
  }
  const ocupada = writeFixture({ actor })
  await assert.rejects(
    () => ocupada.service.create({ uid: 1 }, { email: 'x@aloha.invalid', rol: 'coach', centro_id: 12, coach_id: 502 }),
