...
test('el nombre descargado distingue centro, mes y tamaño', () => {
...
test('no imprime enlaces individuales de niños ni periodos inválidos',async()=>{
  const base={tipo:'encuesta',centro:'Anclas',enlace:`https://example.com/encuesta/${'a'.repeat(48)}`,anio:2026,mes:9}
  await assert.rejects(crearCartelPdf({...base,enlace:`${base.enlace}?p=${'b'.repeat(48)}`},assets),/Enlace/)
  await assert.rejects(crearCartelPdf({...base,mes:13},assets),/Periodo/)
