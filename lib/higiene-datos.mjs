...
//    mentir. Al completar los salones se informa fuera de los pendientes,
//    distinguiendo puestos simultáneos de atención en distintos horarios.
//
...
export function higieneDeDatos({
  ...
  const capacidadRegistrada = salones?.complete ? {
    ninos: salones.simultaneousChildren,
    salones: salones.activeRooms,
    verificacionHorariosPendiente: Boolean(capacidad),
    href: `/centro/${centroId}/grupos#salones`,
  } : null
  if (capacidad && !salones?.complete) {
  ...
  // La capacidad ya cargada se informa aparte y no cuenta como pendiente.
  // Las limitaciones que aún dependan de Dirección mantienen un tono neutro.
  ...
  return {
    ...
    cierre: !ordenados.length
      ? (capacidad ? 'Este centro no tiene datos pendientes por cargar; la capacidad de atención por horarios aún no está verificada.' : 'Los datos de este centro están completos para proyectar.')
      : bloqueantes.length
