...
export function higieneDeDatos({
  ...
  if (capacidad) {
    ...
    lista.push({
      ...
      titulo: estimado?.total == null ? 'Capacidad estimada' : `Capacidad estimada: ${estimado.total} cupos`,
      accion: `Es una estimación de cupos totales${cupo ? ` (${cupo})` : ''}. Falta contrastarla con salones, horarios y coaches disponibles. La plataforma todavía no ofrece cómo validarla; por eso la confianza tiene un máximo de MEDIA.`,
      items: [],
      cuantos: 1,
      bloquea: false,
      ganancia: 0,
