...
export function estadoAsOfMes(estudiante, eventosEstudiante, year, month) {
  // Anulación = inscripción revertida, incluso al consultar un mes anterior.
  // Su acción exige abiertos los periodos originales; los cerrados usan foto.
  if (matriculaAnulada(estudiante)) return null
...
export function cuadroDeserciones(estudiantes, eventos, grupos) {
  ...
  for (const ev of eventos || []) {
    ...
    if (!e) continue
    if (matriculaAnulada(e)) continue
