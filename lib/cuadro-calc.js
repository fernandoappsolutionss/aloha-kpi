...
export function movimientosMes(eventos) {
  const nuevosIds = new Set()
  const reincorporadosIds = new Set()
  const retiradosIds = new Set()
  const graduadosIds = new Set()
  for (const ev of eventos || []) {
    if (ev.tipo === 'inscripcion') nuevosIds.add(ev.estudiante_id)
    else if (ev.tipo === 'reincorporacion') reincorporadosIds.add(ev.estudiante_id)
    else if (ev.tipo === 'retiro') {
      retiradosIds.add(ev.estudiante_id)
      if (ev.motivo === 'GRADUADO') graduadosIds.add(ev.estudiante_id)
    }
  }
...
// del mes NO pagan. "Nuevos" = con evento de inscripción en el mes; el resto
// (incluidos los reincorporados) cuenta como "continúan".
export function cuadroRoyalties(estudiantes, eventos, royaltyRate) {
  const { nuevosIds } = movimientosMes(eventos)
  ...
  for (const it of ITINERARIOS) {
    for (let nivel = 1; nivel <= NIVEL_MAX[it]; nivel++) {
      ...
      const nuevos = del.filter((e) => nuevosIds.has(e.id)).length
...
export function cuadroControlGrupos(grupos, estudiantes, eventos) {
  const { nuevosIds, reincorporadosIds, retiradosIds } = movimientosMes(eventos)
  const filas = []
  let continuan = 0, nuevosTot = 0, reincorporadosTot = 0, retiradosTot = 0
  // porNiño + contadores de un conjunto de niños; acumula los totales del centro.
  const armarFila = (kids) => {
    const porNiño = kids
      .map((e) => ({
        ...e,
        esNuevo: nuevosIds.has(e.id),
        esReincorporado: reincorporadosIds.has(e.id),
        esRetirado: e.estado === 'retirado' && retiradosIds.has(e.id),
      }))
    ...
    const nuevos = activosG.filter((e) => e.esNuevo).length
    const reincorporados = activosG.filter((e) => e.esReincorporado).length
    const retirados = porNiño.length - activosG.length
    const totalMes = activosG.length
    continuan += totalMes - nuevos - reincorporados
    nuevosTot += nuevos
    reincorporadosTot += reincorporados
    retiradosTot += retirados
    return { porNiño, contadores: { mesAnterior: totalMes - nuevos - reincorporados + retirados, nuevos, reincorporados, retirados, totalMes } }
  }
  for (const g of grupos || []) {
    const kids = (estudiantes || []).filter((e) => String(e.grupo_id) === String(g.id) &&
      (e.estado === 'activo' || e.estado === 'baja_potencial' || (e.estado === 'retirado' && retiradosIds.has(e.id))))
  ...
  return {
    ...
    totales: {
      ...
      aPagar: continuan + nuevosTot + reincorporadosTot,
      gruposActivos,
    },
  }
}
