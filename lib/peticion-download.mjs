import { canAccessCentro, ADMIN_ROLES } from './current-user.mjs'
...
export function createPeticionDownloadHandler({ authenticate, findQuote, getBlob }) {
  return async function download(cotizacionId) {
    ...
    // Borradores: solo su creador o gerencia pueden descargar (mismo criterio que assertQuoteAccess).
    if (quote.submitted_at === null && !ADMIN_ROLES.has(actor.rol) && String(quote.created_by) !== String(actor.id)) return notFound()
