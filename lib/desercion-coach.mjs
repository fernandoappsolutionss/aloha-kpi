...
//
// 2. LAS ALERTAS usan el propio centro. La comparación global ponderada de
//    todos los centros registrados se añade como segunda referencia en
//    desercion-comparada.mjs, según el pedido de Fernando del 2026-09-07.
//    La media de red no oculta un caso local ni cambia los candados de alerta.
//
...
export async function consultarDesercionPorCoach(sql, { centroId, anio, mesDesde, mesHasta }) {
  const filas = await sql`
    WITH salidas AS (
      ...
      JOIN grupos g ON g.id = e.de_grupo_id AND g.centro_id = e.centro_id
      WHERE e.centro_id = ${centroId} AND e.tipo = 'retiro'
        AND NOT EXISTS (SELECT 1 FROM estudiantes anulados WHERE anulados.id = e.estudiante_id AND anulados.estado = 'matricula_anulada')
    ...
    activos AS (
      ...
      FROM estudiantes s JOIN grupos g ON g.id = s.grupo_id AND g.centro_id = s.centro_id
  ...
  const [huerfanos] = await sql`
    ...
    FROM estudiante_eventos e
    LEFT JOIN grupos g ON g.id = e.de_grupo_id AND g.centro_id = e.centro_id
    LEFT JOIN coaches c ON c.id = g.coach_id AND c.centro_id = e.centro_id AND c.activo = true
    WHERE e.centro_id = ${centroId} AND e.tipo = 'retiro'
      AND NOT EXISTS (SELECT 1 FROM estudiantes anulados WHERE anulados.id = e.estudiante_id AND anulados.estado = 'matricula_anulada')
      AND e.year = ${anio} AND e.month BETWEEN ${mesDesde} AND ${mesHasta}
      AND c.id IS NULL
...
export function alertasDeCoach(filas, opciones = {}) {
  ...
  // En la lectura comparada, el padrón completo del centro también incluye
  // activos y graduados sin coach. El fallback conserva las llamadas antiguas.
  const baseCentro = opciones.baseCentro || { bajasReales: bajasCentro + huerfanas,
    expuestos: expuestosCentro + huerfanas, graduados: graduadosCentro }
  const tasaCentro = baseCentro.expuestos > 0
    ? baseCentro.bajasReales / baseCentro.expuestos
    : 0
  ...
  const coaches = base.map((c) => {
    ...
    const candados = {
      ...
      margenRelativo: baseCentro.bajasReales > 0 && c.bajasReales >= CANDADOS.razonMin * esperadas,
