...
export function analyze(moveKids, srcGrupo, tgtGrupo, { MIN, MAX }) {
  ...
  // Regla de Fernando (2026-08-01): NO se fusiona Tiny con Kids — el manual lo
  // permitía desde KIDS nivel 3, pero en la práctica una fusión mixta es muy
  // complicada de manejar. Aplica por niño movido contra el itinerario destino.
  const tiposMov = new Set(moveKids.map((e) => e.itinerario).filter((it) => it === 'TINY' || it === 'KIDS'))
  if ((tgtGrupo.itinerario === 'KIDS' && tiposMov.has('TINY')) ||
      (tgtGrupo.itinerario === 'TINY' && tiposMov.has('KIDS'))) {
    blocked = true
    reasons.push({ t: 'No se fusiona Tiny con Kids (regla del negocio)', k: 'no' })
  }
...
export function sugerenciasPara(grupoOrigen, todosGrupos, opts) {
  ...
  for (const g of todosGrupos) {
    ...
    // Tiny no se cruza con Kids ni como opción en el listado (regla del negocio).
    if ((g.itinerario === 'KIDS' && tiposMov.has('TINY')) ||
        (g.itinerario === 'TINY' && tiposMov.has('KIDS'))) continue
