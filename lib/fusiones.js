...
import { TINYMAP } from './operaciones.js'
...
// (R3, remodelado 2026-08-08) El cierre es POR NIÑO: override manual ??
// derivado del plan del niño — el enriquecido de lib/plan-nino trae
// plan.cierre = cierreEfectivo (ya resuelto override ?? derivado); un niño sin
// enriquecer solo aporta su override crudo (fecha_cierre_nivel). MURIÓ el
// fallback al cierre estimado del itinerario del grupo: el grupo no cierra por
// promedio, cada niño cierra su nivel en su propia fecha. Pesos intactos.
const cierreNino = (e) => e?.plan?.cierre?.fecha || e?.fecha_cierre_nivel || null
export function cierreScore(moveKids, tgtKids) {
  const fechas = (arr) => (arr || [])
    .map((e) => {
      const f = cierreNino(e)
      const d = f && new Date(f)
      return d && !isNaN(d) ? d : null
    })
    .filter(Boolean)
  const fa = fechas(moveKids)
  const fb = fechas(tgtKids)
...
export function analyze(moveKids, srcGrupo, tgtGrupo, { MIN, MAX }) {
  ...
  // Cierre por niño (override ?? derivado del plan enriquecido): ya no se pasa
  // el cierre estimado de los grupos — ese fallback murió (R3).
...
// Candidatos de fusión para un grupo: activos, con la palanca de inscripciones
// abierta, no online, no Kinder, con niños. Si el origen mueve niños de nivel
// 3+ se excluyen los destinos base 1–2.
...
export function sugerenciasPara(grupoOrigen, todosGrupos, opts) {
  ...
  for (const g of todosGrupos) {
    ...
    // Defecto D3: un DESTINO con la palanca cerrada no se sugiere (cerrada =
    // no entra nadie; aplicarFusion lo rechazaría igual). El grupo cerrado sí
    // puede ser ORIGEN: sacar niños de él nunca está bloqueado.
    if (g.inscripcion_abierta === false) continue
