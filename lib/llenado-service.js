...
export async function encolarSyncCrm(grupoIds, motivo) {
  const ids = dedupIds(grupoIds)
  if (!ids.length) return { encoladas: 0 }
  const eventos = await sql`
    SELECT crm_event_id, grupo_id FROM centro_eventos WHERE grupo_id = ANY(${ids})
  `
  const lote = armarLoteOutbox(eventos, { op: 'sync_group', motivo, clave: new Date().toISOString() })
