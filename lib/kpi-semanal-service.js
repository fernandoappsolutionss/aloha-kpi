...
async function cargarDatosCentro(centroId, query) {
  const estudiantes = await query`
    SELECT id, grupo_id, estado, fecha_inscripcion, origen_venta, crm_registration_id, ultima_asistencia
...
function calcularConDatos({ estudiantes, grupos, eventos }, year, month) {
  const auto = calcularKpiSemanalAuto({
    year,
    month,
    inscripciones: eventosSinMatriculasAnuladas(estudiantes, eventos).filter((evento) => evento.tipo === 'inscripcion'),
