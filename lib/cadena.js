...
// en pantalla.
export async function cierreMesAnterior(centroId, year, month, query = sql) {
  ...
  `
  if (!prev || prev.ninos_final_mes == null) return null
  const valor = Number(prev.ninos_final_mes)
  return Number.isFinite(valor) ? { valor, year: py, month: pm } : null
}
