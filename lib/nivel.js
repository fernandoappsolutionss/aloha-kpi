import { CENTER_LEVELS } from './growth/constants.mjs'
...
// El umbral es "IGUAL O MAYOR A XX" (>=): el valor exacto del límite ya otorga el nivel.
//   Nivel 1: ≥ 170 niños   ·  Nivel 2: ≥ 200   ·  Nivel 3: ≥ 230
//   Nivel 4: ≥ 325 niños   ·  Nivel 5: ≥ 410
export const NIVELES = [...CENTER_LEVELS]
  .reverse()
  .map(({ level, threshold }) => ({ nivel: level, min: threshold }))
...
// Se otorga con "igual o mayor a" el umbral (>=), no solo mayor que.
...
// Devuelve { nivel, min, faltan } o null si ya alcanzó el nivel máximo.
// Como el nivel se otorga con >=, alcanzar el umbral exacto ya cuenta: el
// siguiente nivel es el primero cuyo umbral sea estrictamente mayor a los niños.
export function siguienteNivel(ninos) {
  const asc = [...NIVELES].sort((a, b) => a.min - b.min)
  for (const n of asc) {
    if (ninos < n.min) return { nivel: n.nivel, min: n.min, faltan: n.min - ninos }
  }
  return null
}
