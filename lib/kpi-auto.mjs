...
export function fuenteKpiAutomatica({ year, month, clases = [], registros = [], movimientos = [] }) {
  ...
  for (const [movementIndex, movement] of movimientos.entries()) {
    ...
    if (movement.tipo === 'inscripcion') {
      ...
      source[origin] += 1
      const rawId = movement.movimiento_id ?? movement.estudiante_id ?? movement.crm_registration_id
      source._origin_sales.push({
        id: rawId == null || rawId === '' ? `posicion:${movementIndex}` : String(rawId),
        origin,
      })
...
function validOriginSales(source) {
  ...
  if (source._origin_sales.length !== matrixTotal(source.ing)) return null
  const ids = new Set()
...
function targetFromExistingAdjustments(source, adjustments) {
  const target = { ing: source.ing }
  for (const key of ORIGIN_FIELDS) {
    target[key] = Math.max(0, intOrZero(source[key]) + intSigned(adjustments[key]))
  }
  const total = matrixTotal(source.ing) + matrixTotal(adjustments.ing)
  let remaining = total
  for (const key of ORIGIN_FIELDS.filter((field) => field !== 'orig_por_clasificar')) {
    const amount = Math.min(remaining, target[key])
    target[key] = amount
    remaining -= amount
  }
  target.orig_por_clasificar = remaining
...
export function enriquecerAjustesOrigen(adjustments = {}, source = {}) {
  if (adjustments?.[ORIGIN_RECONCILIATION_KEY]?.version === 1) return adjustments
  const metadata = createOriginReconciliation(source, targetFromExistingAdjustments(source, adjustments))
  return metadata ? { ...adjustments, [ORIGIN_RECONCILIATION_KEY]: metadata } : adjustments
}
