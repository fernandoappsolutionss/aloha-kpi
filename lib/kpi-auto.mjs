...
//     declarado a mano en motivos y orígenes; el embudo CP usa la fuente viva.
//     ing/des jamás se
//     concilian: en mes abierto la tabla dejó de ser su fuente.
import { filtrarClasesPorMes, filtrarClasesPorMomento, resumirClases } from './clases-prueba.mjs'
...
export function fuenteKpiAutomatica({ year, month, clases = [], ventas = [], retiros = [], timeZone = 'America/Panama', now = new Date() }) {
  const source = emptySource()
  source._origin_sales = []
  source.ventasTotal = ventas.length
  const mes = `${Number(year)}-${String(Number(month)).padStart(2, '0')}`
  const clasesMes = filtrarClasesPorMes(clases, mes, timeZone)
  const realizadas = filtrarClasesPorMomento(clasesMes, 'realizadas', now)
  const totals = resumirClases(realizadas)
  // El CRM ya cuenta cancelados, pendientes y asistentes en las tarjetas.
  // Usar esa misma foto evita mezclar fecha de registro o de marcado con
  // el mes de la clase, y no transporta datos personales de cada invitado.
  source.cp_invitados = totals.total
  source.cp_asistieron = totals.attended
  source.cp_matriculados = totals.won
  source._trial_attendance = { basis: 'occurred_event_month', month: mes, classes: realizadas.length }
...
export function crearAjustes(saved = {}, source = {}) {
  ...
  if (source._trial_attendance) {
    adjustments.cp_invitados = 0
    adjustments.cp_asistieron = 0
    adjustments.cp_matriculados = 0
  }
...
export function aplicarAjustes(source = {}, adjustments = {}, saved = {}) {
  ...
  if (source._trial_attendance) {
    result.cp_invitados = intOrZero(source.cp_invitados)
    result.cp_asistieron = intOrZero(source.cp_asistieron)
    result.cp_matriculados = intOrZero(source.cp_matriculados)
    result._trial_attendance = source._trial_attendance
  }
