...
export function fuenteKpiAutomatica({ year, month, clases = [], registros = [], movimientos = [] }) {
  ...
  for (const registration of registros) {
    ...
    if (!id || seenRegistrations.has(id) || registration.attendance_status === 'cancelled') continue
...
export function aplicarAjustes(source = {}, adjustments = {}) {
  ...
  for (const key of CAMPOS_RESUMEN_AUTO) {
    const adjustment = ORIGIN_FIELDS.includes(key)
      ? intSigned(adjustments[key])
      : intOrZero(adjustments[key])
    result[key] = Math.max(0, intOrZero(source[key]) + adjustment)
  }
  for (const type of ['ing', 'des']) {
    for (let week = 0; week < 5; week++) {
      for (let day = 0; day < 5; day++) {
