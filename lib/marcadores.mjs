...
// Cobranza vencida del mes: el PEOR CIERRE DE SEMANA declarado.
//
// Fórmula ALOHA: la semana vale lo que marca su último día registrado; el mes,
// la peor de sus semanas. Antes se tomaba el máximo de TODOS los días y el
// Resumen mostraba el pico del día siguiente al vencimiento de la mensualidad
// (Brisas Q3-2026: "49 vencidas" el 31-jul, cuando el 29-jul eran 8), un número
// que la administradora no reconoce en Zoho. Y antes de eso se leía sólo el
// último día de la última semana saltándose los ceros, lo que aprobaba por el
// final a CALLE 50 y CONDADO; eso sigue corregido: se juzga la PEOR semana.
//
// Un mes sin ningún `cob_*` escrito devuelve null = DESCONOCIDO, no 0.
// Cero vencidas hay que declararlo; no se regala por silencio.
export function cobranzaDeclarada(semanas) {
  const cierres = []
  for (const semana of semanas || []) {
    const valores = DIAS_COBRANZA
      .map((dia) => semana?.[dia])
      // null/undefined/'' = no registrado. El 0 SÍ es una declaración.
      .filter((v) => v !== null && v !== undefined && v !== '')
      .map(Number)
      .filter(Number.isFinite)
    if (!valores.length) continue
    // ponytail: el formulario y el cron guardan en 0 los días aún sin llenar
    // (`21,0,0,0,0`), así que un 0 DESPUÉS de un valor no se distingue de un
    // blanco: el cierre es el último día distinto de cero, y la semana vale 0
    // sólo si todos sus días son 0. Techo: una semana que de verdad baja a 0
    // el viernes se lee con el jueves. Arreglo de fondo: guardar null en vez
    // de 0 para los días no registrados.
    const noCero = valores.filter((n) => n !== 0)
    cierres.push(noCero.length ? noCero[noCero.length - 1] : 0)
  }
