...
export function decisionEmail({ peticion, estado, actor, cotizacionAprobada, centroNombre, baseUrl }) {
  ...
  const cotizacionBlock = aprobada && cotizacionAprobada
    ? `<p style="margin:12px 0;padding:10px;background:#F0FDF4;border-radius:8px;font-size:13px">
        <strong>Cotización aprobada:</strong> ${escapeHtml(cotizacionAprobada.proveedor_razon_social)}${cotizacionAprobada.proveedor_pais ? ` (${escapeHtml(cotizacionAprobada.proveedor_pais)})` : ''} — ${escapeHtml(cotizacionAprobada.archivo_nombre)}
       </p>`
    : aprobada && peticion.proveedor_preaprobado
      ? `<p><strong>Proveedor aprobado del centro:</strong> ${escapeHtml(peticion.proveedor_preaprobado_nombre)}</p>`
      : ''
