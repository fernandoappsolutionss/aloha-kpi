...
// orígenes cuadren con las ventas del KPI semanal sin recorrer
...
export function filtrarClasesVigentesCrm(clases = [], eventosCrm = []) {
  const delCentro = new Set((clases || []).map(event => String(event?.id || '')).filter(Boolean))
  return (eventosCrm || []).filter(event => delCentro.has(String(event?.id || '')))
}
...
export async function cargarFuenteKpi(centroId, year, month, { query = sql, crm = crmCall, now = new Date() } = {}) {
  try {
    ...
    return {
      ...
      source: fuenteKpiAutomatica({
        ...
        clases,
        now,
        timeZone: Number(centroId) === 10 ? 'America/Caracas' : 'America/Panama',
        ventas: auto.ventas,
...
async function calcularAjustesKpi(centroId, year, month, source, query = sql) {
  ...
  const [resumen] = await query`
    ...
    WHERE centro_id = ${centroId} AND year = ${year} AND month = ${month}
  `
  return crearAjustes(formaKpiGuardada(resumen), source)
}

export async function inicializarAjustesKpi(centroId, year, month, source, query = sql) {
  const adjustments = await calcularAjustesKpi(centroId, year, month, source, query)
  const [existing] = await query`
    SELECT ajustes FROM kpi_auto_ajustes
    WHERE centro_id = ${centroId} AND year = ${year} AND month = ${month}
  `
  if (existing) return parseJson(existing.ajustes, adjustments)
...
function hasAdjustment(adjustments) {
  return Object.entries(adjustments || {}).some(([key, value]) => {
    ...
    if (key === 'cp_invitados' || key === 'cp_asistieron' || key === 'cp_matriculados') return false
...
export async function fotoKpiAutomatica(centroId, year, month, options = {}) {
  ...
  const conciliar = async (query) => {
    const adjustments = shouldPersist
      ? await inicializarAjustesKpi(centroId, year, month, loaded.source, query)
      : await calcularAjustesKpi(centroId, year, month, loaded.source, query)
  ...
  try {
    if (!shouldPersist) return await conciliar(sourceQuery)
    if (options.query) return await conciliar(options.query)
    // Reintentar solo la transacción propia: otro snapshot, misma fuente CRM.
