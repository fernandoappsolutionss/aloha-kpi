...
export function mezclarSemanasPeriodoAutomaticas(semanas = [], centroId, year, month, automatic = {}) {
  const isTarget = (row) => (
    Number(row.centro_id) === Number(centroId) &&
    Number(row.year) === Number(year) &&
    Number(row.month) === Number(month)
  )
  const current = (semanas || []).filter(isTarget)
  const other = (semanas || []).filter((row) => !isTarget(row))
  const automaticWeeks = mezclarSemanasAutomaticas(current, automatic).map((row) => ({
    ...row,
    centro_id: Number(centroId),
    year: Number(year),
    month: Number(month),
  }))
  return [...other, ...automaticWeeks]
}

const totalMatrizAutomatica = (matrix = []) => (matrix || []).reduce(
...
export function mezclarTotalesAutomaticos(totales = [], year, month, automatic = {}) {
  const automaticRow = {
    year: Number(year),
    month: Number(month),
    nuevos_ingresos_venta: totalMatrizAutomatica(automatic.ing),
    total_desercion: totalMatrizAutomatica(automatic.des),
  }
  let found = false
  const merged = (totales || []).map((row) => {
    if (Number(row.year) !== Number(year) || Number(row.month) !== Number(month)) return row
    found = true
    return { ...row, ...automaticRow }
  })
  return found ? merged : [...merged, automaticRow]
}
