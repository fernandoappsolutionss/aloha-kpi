...
async function cargarAssets() {
  if (!assetsPromise) assetsPromise = Promise.all([
    '/carteles/logo-oficial.png', '/fonts/FuturaMdBT.ttf', '/fonts/FuturaMdBT-Bold.ttf',
  ].map(async path => {
    const res = await fetch(path)
    if (!res.ok) throw new Error('No se pudo cargar el diseño ALOHA. Revisa la conexión e intenta nuevamente.')
    return res.arrayBuffer()
  })).then(([logo,regular,bold])=>({logo,regular,bold})).catch(error=>{assetsPromise=null;throw error})
