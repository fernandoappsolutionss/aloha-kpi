...
// Ventanas de las poses completas en la lámina vectorial oficial, medidas
// sobre su vista de 1888 × 1334. No se redibuja, deforma ni refleja al personaje.
const POSES_MONO = {
  encuesta: [1106, 1198, 1204, 1289], // con teléfono
  google: [1228, 974, 1319, 1070], // saludando
  wifi: [1450, 1154, 1575, 1252], // bienvenida con ábaco
}
export async function crearCartelPdf({ tipo, centro, enlace, formato = 'carta', red = '', clave = '' }, assets) {
  ...
  if (tipo === 'encuesta') {
    ...
    if (!['https:', 'http:'].includes(url.protocol) || !/^\/encuesta\/centro\/[a-f0-9]{48}$/.test(url.pathname) || url.username || url.password || url.search || url.hash) throw new Error('Enlace permanente de encuesta inválido.')
  }
  ...
  const original = (await PDFDocument.load(assets.mono)).getPage(0)
  const [left, top, right, bottom] = POSES_MONO[tipo]
  const fx = original.getWidth()/1888, fy = original.getHeight()/1334
  const mono = await doc.embedPage(original, {
    left:left*fx, right:right*fx, top:original.getHeight()-top*fy, bottom:original.getHeight()-bottom*fy,
  })
  ...
  const wifiConQr = tipo === 'wifi' && Boolean(destino)
  const escalaMono = Math.min(218/mono.width, (wifiConQr ? 204 : 244)/mono.height)
  const monoW = mono.width*escalaMono, monoH = mono.height*escalaMono
  page.drawPage(mono, {x:42+(218-monoW)/2, y:h-(wifiConQr ? 535 : 581), width:monoW, height:monoH})
  if (destino) {
    const matrix = QRCode.create(destino, { errorCorrectionLevel: 'M' }).modules
    const size = tipo === 'wifi' ? 210 : 254, unit = size/(matrix.size+8), x = w-42-size, y = h-326-size
    ...
    for (const r of [35, 61, 88]) {
      page.drawSvgPath(`M ${-r*0.82} ${-r*0.57} Q 0 ${-r*1.43} ${r*0.82} ${-r*0.57}`, {x:w-157, y:h-493, borderWidth:11, borderColor:azul})
    }
    page.drawCircle({ x:w-157, y:h-490, size:10, color:verde })
  }
  ...
  if (tipo === 'encuesta') {
    texto('Encuesta de satisfacción', h-676, 16, bold)
    texto('Use el nombre de su hijo y el teléfono registrado en ALOHA.', h-699, 13)
    texto('El mismo QR abre la encuesta de cada mes. Gracias por participar.', h-719, 12)
  } else if (tipo === 'google') {
