...
export const TIPOS_SEMANA = {
  ...
  evaluacion: { label: 'Evaluación', desc: 'Evaluación completa (cada 3 semanas)' },
  mental: { label: 'Mental Day', desc: 'Día de cálculo mental' },
  cierre: { label: 'Cierre', desc: 'Repaso, examen y cierre de nivel' },
}

// Plantilla de semanas de un nivel (~22 semanas: tope del rango 19–22 del
// manual). Es la base de franquicia: igual para todos los centros; lo que se
// ajusta por grupo son las excepciones (clases suspendidas) y la fecha de inicio.
// Cada semana: { etiqueta, tipo, corto } — `corto` es la etiqueta de la línea
// de tiempo (I1, 7, MD1, C2).
export function plantillaNivel(nivel) {
  const md1TrasSemana = Number(nivel) === 1 ? 4 : 5
  const semanas = [
    { etiqueta: 'Inducción 1', tipo: 'induccion', corto: 'I1' },
    { etiqueta: 'Inducción 2', tipo: 'induccion', corto: 'I2' },
  ]
  let md = 0
  for (let s = 1; s <= 15; s++) {
    const evalua = s % 3 === 0
    semanas.push({ etiqueta: `Semana ${s}${evalua ? ' · Evaluación' : ''}`, tipo: evalua ? 'evaluacion' : 'clase', corto: String(s) })
    if (s === md1TrasSemana) semanas.push({ etiqueta: 'Mental Day 1', tipo: 'mental', corto: `MD${++md}` })
    if (s === 9) semanas.push({ etiqueta: 'Mental Day 2', tipo: 'mental', corto: `MD${++md}` })
  }
  semanas.push(
    { etiqueta: 'Cierre 1 · Repaso y examen', tipo: 'cierre', corto: 'C1' },
    { etiqueta: 'Cierre 2 · Examen', tipo: 'cierre', corto: 'C2' },
    { etiqueta: 'Cierre 3 · Cierre de nivel', tipo: 'cierre', corto: 'C3' },
  )
  return semanas
}
...
export function generarItinerario({ fechaInicio, dias, nivel, conFeriados = true, excepciones = [] }) {
