export const MESES = { ENE: '01', JAN: '01', FEB: '02', MAR: '03', ABR: '04', APR: '04', MAY: '05', JUN: '06', JUL: '07', AGO: '08', AUG: '08', SEP: '09', SET: '09', OCT: '10', NOV: '11', DIC: '12', DEC: '12' }
...
// ISO de una fecha que existe de verdad (31-FEB no); null si no existe.
export function isoSiExiste(a, m, d) {
  const f = new Date(Date.UTC(a, m - 1, d))
  if (f.getUTCFullYear() !== a || f.getUTCMonth() + 1 !== m || f.getUTCDate() !== d) return null
...
function fechaFila(dd, mmm, yy) {
  ...
  const iso = isoSiExiste(2000 + Number(yy), Number(mes), Number(dd))
  if (!iso) throw new Error(`Fecha inválida en el estado de cuenta: ${dd}-${mmm}-${yy}.`)
  return iso
}
...
export function parseStGeorgesTexto(texto) {
  ...
  return {
    ...
    descuadre,
    movimientos: conFitidStGeorges(movimientos),
  }
}

// Filas de St. Georges (PDF o Excel) ordenadas de la más vieja a la más nueva, con el
// saldo corrido ya verificado → movimientos con fitid. El PDF y el Excel del mismo
// periodo producen el mismo fitid, así que se deduplican entre sí.
export function conFitidStGeorges(filas) {
  const vistos = new Map()
  // Una fila en 0.00 (mayo 2026: "ITBMS CLAVE ..." sin monto) no mueve caja y la tabla
  // exige monto <> 0; ya participó en el saldo corrido. El OFX hace lo mismo.
  return filas.filter((m) => m.monto !== 0).map(({ fecha, monto, memo, saldo }) => {
    // Sin ID del banco: fecha + saldo corrido + monto identifica la fila. Si el saldo vuelve
    // a un valor previo el mismo día (+50, -50, +50), la repetición lleva sufijo |2, |3...
    // La primera ocurrencia queda sin sufijo para que archivos solapados sigan deduplicando.
    const base = `${fecha}|${saldo.toFixed(2)}|${monto.toFixed(2)}`
    const n = (vistos.get(base) || 0) + 1
    vistos.set(base, n)
    return { fecha, monto, memo, fitid: n === 1 ? base : `${base}|${n}` }
  })
}
