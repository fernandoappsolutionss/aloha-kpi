...
function fechaFila(dd, mmm, yy) {
  ...
  const [a, m, d] = [2000 + Number(yy), Number(mes), Number(dd)]
  const f = new Date(Date.UTC(a, m - 1, d))
  if (f.getUTCFullYear() !== a || f.getUTCMonth() + 1 !== m || f.getUTCDate() !== d) {
    throw new Error(`Fecha inválida en el estado de cuenta: ${dd}-${mmm}-${yy}.`)
  }
  return `20${yy}-${mes}-${dd}`
}
...
export function parseStGeorgesTexto(texto) {
  ...
  return {
    ...
    // exige monto <> 0; ya participó en el saldo corrido de arriba. El OFX hace lo mismo.
    movimientos: movimientos.filter((m) => m.monto !== 0).map(({ fecha, monto, memo, saldo }) => {
