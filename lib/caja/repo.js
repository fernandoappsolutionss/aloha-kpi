  ...
  return sql`
    ...
    LEFT JOIN LATERAL (
      SELECT x.monto, x.fecha FROM (
        SELECT c.saldo_ancla AS monto, c.saldo_ancla_fecha AS fecha WHERE c.saldo_ancla IS NOT NULL
        UNION ALL
        SELECT i.saldo_banco, i.saldo_banco_fecha FROM caja_importaciones i WHERE i.cuenta_id = c.id AND i.saldo_banco IS NOT NULL
      ) x ORDER BY x.fecha DESC LIMIT 1
    ) b ON true
    WHERE c.activa
...
export async function clasificarMovimientoRepo({ id, clase, categoria, patron, usuarioId }) {
  return withTransaction(async (query) => {
    ...
    if (patron) {
      ...
      if (ids.length) {
        await query`UPDATE caja_movimientos SET clase = ${clase}, categoria = ${categoria}, regla_id = ${regla.id}, clasificado_por = ${usuarioId} WHERE id = ANY(${ids}::int[])`
      }
      otros = ids.length
    }
