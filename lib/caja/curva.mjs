...
// Sin un solo mes cerrado de extractos el perfil propio proyecta 0 de cobro y la
// curva se pinta roja por falta de datos, no por falta de caja: se usa el perfil
// de referencia (Zoho) y se marca `estimado` para que la pantalla lo diga.
export function perfilConRespaldo(perfil, referencia) {
  if (!perfil?.meses?.length && referencia) return { ...referencia, pesos: [...referencia.pesos], meses: [], estimado: true }
  return { ...perfil, estimado: false }
}

// Uso de la línea de crédito del grupo. Los giros y abonos se ven en los extractos
// de las cuentas OPERATIVAS (clase 'linea'): un giro entra como +monto (sube el uso)
// y un abono sale como −monto (lo baja). Se parte del saldo base de la cuenta de la
// línea y se suma lo posterior a esa fecha, de las dos empresas. Los movimientos del
// extracto de la propia línea ya se reflejan en su saldo base: no se cuentan doble.
export function lineaUsada(linea, movimientos = []) {
  if (!linea) return 0
  const base = Number(linea.base) || 0
  const desde = linea.baseFecha || ''
  const delta = movimientos
    .filter((m) => m.clase === 'linea' && m.cuenta_tipo !== 'linea_credito' && m.fecha > desde)
    .reduce((s, m) => s + (Number(m.monto) || 0), 0)
  return r2(Math.max(0, base + delta))
}
...
export function calcularCurva({ hoy, saldoHoy, perfil, compromisos = [], ajustes = [], baldes = [], baldesEstado = { duenoPendiente: 0, reservaImpuesto: 0 }, lineaDisponible = 0 }) {
  ...
  // Coerción como en el resto de la función: un NUMERIC de Neon o un input de
  // formulario llega como texto y `saldo + ingresos` lo concatenaría.
  const lineaNum = Number(lineaDisponible) || 0
  const duenoPendiente = Number(baldesEstado?.duenoPendiente) || 0
  let saldo = Number(saldoHoy) || 0
  let reserva = Number(baldesEstado?.reservaImpuesto) || 0
  const semanas = lunes.map((l, i) => {
    ...
    const dueno = (ingresos * pctDueno) / 100 + (i === 0 ? duenoPendiente : 0)
    const egresos = {}
  ...
  return conSemaforo(semanas, lineaNum)
}
