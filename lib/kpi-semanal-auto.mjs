...
export function calcularKpiSemanalAuto({ year, month, inscripciones = [], retiros = [] } = {}) {
  ...
  let desTotal = 0
  let traslados = 0
  // Las MISMAS filas que alimentaron las matrices, para que los derivados de
  // resumen (orig_*, mot_*, cp_matriculados) se calculen sobre exactamente las
  // ventas y retiros que contó este motor — una sola fuente de derivación.
  const ventasContadas = []
  ...
  return {
    ...
    ingTotal,
    desTotal,
    ventas: ventasContadas,
    retiros: retirosContados,
