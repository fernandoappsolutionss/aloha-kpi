...
async function loadGrowthInputs(centroId) {
  ...
  const [centerRows, summaries, states, weeklyRows, reincorporations, students, groups, events, salons, metaRows] = await Promise.all([
    ...
    `,
    sql`
      SELECT id, grupo_id, estado, fecha_inscripcion, created_at, retiro_programado_para, origen, crm_registration_id, ultima_asistencia
    ...
    sql`
      ...
      ORDER BY fecha, id
    `,
    sql`SELECT id, nombre, activo, capacidad_ninos FROM salones WHERE centro_id = ${centroId}`,
