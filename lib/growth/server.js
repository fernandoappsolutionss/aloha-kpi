...
import { buildGrowthHistory, buildOperationalGrowth, selectCurrentPopulation } from './source.mjs'
...
async function persistGrowth(centroId, snapshotDate, payload, recommendations) {
  ...
  await sql`
    ...
    WHERE centro_id = ${centroId}
      AND generated_for = ${snapshotDate}
      AND status = 'pending'
  `

  const persisted = []
  for (const item of recommendations) {
    const dueDate = addDays(snapshotDate, item.dueDays)
    ...
    const [row] = await sql`
      INSERT INTO growth_recommendations (
        ...
        ${centroId}, ${snapshot.id}, ${item.kind}, ${snapshotDate}, ${item.title},
      ...
      ON CONFLICT (centro_id, kind, generated_for) DO UPDATE SET
        ...
        status = CASE
          WHEN growth_recommendations.status IN ('completed', 'dismissed')
...
export async function calculateCentroGrowth(centroId, { persist = true } = {}) {
  ...
  const latestSummary = history
    .filter((row) => `${row.year}-${String(row.month).padStart(2, '0')}` <= operational.currentPeriod)
    .at(-1)
  const population = selectCurrentPopulation({
    ...
    latestSummaryChildren: latestSummary?.ninos_final_mes,
  })
