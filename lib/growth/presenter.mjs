...
export function confidenceMeta(level) {
  if (level === 'high') return { label: 'Datos completos', tone: 'ok' }
  if (level === 'medium') return { label: 'Datos por revisar', tone: 'warn' }
  return { label: 'Datos insuficientes', tone: 'bad' }
}

export function scenarioChartRows(projection, { history = [], currentPeriod, startPeriod } = {}) {
  ...
  const observed = new Map(history.filter(row => row.closed)
    .map(row => [`${row.year}-${String(row.month).padStart(2, '0')}`, row.endChildren]))
  const historicalPeriods = []
  if (parsePeriod(currentPeriod)) {
    if (parsePeriod(startPeriod)) {
      const cursor = new Date(`${startPeriod}-01T12:00:00Z`)
      while (cursor.toISOString().slice(0, 7) < currentPeriod && historicalPeriods.length < 24) {
        historicalPeriods.push(cursor.toISOString().slice(0, 7))
        cursor.setUTCMonth(cursor.getUTCMonth() + 1)
      }
    }
    for (const map of Object.values(maps)) map.set(currentPeriod, projection?.currentChildren ?? null)
  }
  const periods = [...new Set([...historicalPeriods,
  ...
  return periods.map((period) => ({
    ...
    ...(currentPeriod ? { observed: period < currentPeriod ? observed.get(period) ?? null : null } : {}),
  }))
}
