...
export function buildGrowthPayload(input, { recommendationStates = [] } = {}) {
  ...
  if (population.difference !== 0) issues.push({
    ...
    message: `El saldo mensual (${population.expectedMonthEndChildren}) difiere del padrón elegible (${rosterExpected}) en ${Math.abs(population.difference)} niños. Revisa grupos y movimientos.`,
  })
