...
import { roomCapacitySummary } from '../salon-capacidad.mjs'
...
// El vínculo de cada venta con una inscripción se exige desde septiembre.
// Los KPI anteriores se recibieron agregados y no requieren carga retroactiva.
const INDIVIDUAL_SALES_START_PERIOD = '2026-09'
...
export function buildGrowthHistory({ summaries = [], states = [], weekly = [], reincorporations = [], students = [], events = [] }) {
  ...
  for (const event of eventosSinMatriculasAnuladas(students, events).filter((item) => item.tipo === 'inscripcion').sort((a, b) =>
  ...
  return summaries
    .map((summary) => {
      ...
      const issues = []
      // `period` no cambia ninguna decisión del motor: existe para que la
      // alerta de higiene pueda decir QUÉ MES falta clasificar. Sin él, seis
      // meses incompletos se veían como una sola frase genérica repetida, que
      // es la definición de papel tapiz.
      if (key >= INDIVIDUAL_SALES_START_PERIOD && !reliable) issues.push({
        code: 'cp_classification_incomplete', severity: 'warning', period: key,
        message: 'Las inscripciones vinculadas a fichas y clasificadas como clase de prueba o directas no cubren todas las ventas del mes; se conserva la matrícula declarada.',
      })
...
export function buildOperationalGrowth({ today, students = [], groups = [], events = [], salons = [], groupCapacity = null }) {
  ...
  return {
    ...
    // Los puestos simultáneos no son un máximo de matriculados: un salón
    // atiende varios grupos en distintos horarios. No limitar la ruta con esa suma.
    roomCapacity: roomCapacitySummary(salons),
    capacityMax: null,
    capacityEstimate: {
