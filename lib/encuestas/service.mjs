...
export function crearServicioEncuestas({ query, transaction, now = () => new Date() }) {
  ...
  async function cargar(centroId, anio, mes) {
    ...
    return {centro,anio,mes,abierta:vigente,campana:{id:c.id,token:c.token,corte:c.corte instanceof Date ? c.corte.toISOString().slice(0,10) : String(c.corte).slice(0,10),version:c.version,compartida_at:c.compartida_at},
  ...
  return {cargar,preparar,registrarDifusion,publica,responder}
}
