...
import { VERSION, REGLA_PARTICIPACION, periodoPanama, periodoAbierto, validarPeriodo, validarRespuesta, normalizarNombre, telefonoIdentidad, resumenEncuesta } from './domain.mjs'
...
export function crearServicioEncuestas({ query, transaction, now = () => new Date() }) {
  ...
  async function mesCerrado(q, c) {
    const [m] = await q`SELECT estado FROM mes_kpi WHERE centro_id=${c.centro_id} AND year=${c.anio} AND month=${c.mes}`
    return Boolean(m && m.estado !== 'abierto')
  }
  async function exigirAbierto(q, c) {
    if (!abierto(c)) throw new Error('Esta encuesta está cerrada. Solicita al centro el enlace del mes actual.')
    // Mismo candado mensual que el cierre KPI: validar y responder/difundir
    // sucede antes o después del cierre, nunca entre su lectura y escritura.
    await q`INSERT INTO mes_kpi(centro_id,year,month,estado) VALUES(${c.centro_id},${c.anio},${c.mes},'abierto')
      ON CONFLICT(centro_id,year,month) DO NOTHING`
    const [m] = await q`SELECT estado FROM mes_kpi WHERE centro_id=${c.centro_id} AND year=${c.anio} AND month=${c.mes} FOR UPDATE`
    if (m?.estado !== 'abierto') throw new Error('Esta encuesta está cerrada. Solicita al centro el enlace del mes actual.')
  }
  ...
  async function cargar(centroId, anio, mes) {
    ...
    const vigente = periodoAbierto(anio,mes,now()) && !await mesCerrado(query,{centro_id:centroId,anio,mes})
    if (!c) {
  ...
  async function preparar(centroId, anio, mes) {
    ...
    return transaction(async q => {
      ...
      await exigirAbierto(q,{centro_id:centroId,anio,mes})
      ...
      const [c] = await q`INSERT INTO encuesta_campanas(centro_id,anio,mes,token,identity_salt,version,activos,corte,regla_participacion)
        VALUES(${centroId},${anio},${mes},${tokenNuevo()},${salt},${VERSION},${estudiantes.length},${periodoPanama(now()).hoy},${REGLA_PARTICIPACION}) RETURNING id,token`
  ...
  async function publica(token, individual = null) {
    ...
    const [c] = await query`SELECT c.id,c.centro_id,c.anio,c.mes,c.version,ce.nombre FROM encuesta_campanas c JOIN centros ce ON ce.id=c.centro_id WHERE c.token=${token}`
    if (!c) return null
    if (individual) {
    ...
    return {nombre:c.nombre,anio:c.anio,mes:c.mes,version:c.version,abierta:abierto(c) && !await mesCerrado(query,c),individual:Boolean(individual)}
  }
