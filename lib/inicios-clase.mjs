...
import { grupoOperativoConNinos } from './operaciones.js'
...
export const INICIOS_CLASE_DESDE_FECHA = '2026-08-01'
...
function contextoInicio(estudiante, gruposPorId, eventosEstudiante) {
  const eventosOrdenados = [...eventosEstudiante].sort(compareEvents)
  const eventoInscripcion = eventosOrdenados.find((evento) => evento.tipo === 'inscripcion') || null
  const fechaInscripcion = iso10(eventoInscripcion?.fecha) || iso10(estudiante?.fecha_inscripcion)
  let eventoAsignacion = eventoInscripcion?.a_grupo_id ? eventoInscripcion : null

  if (!eventoAsignacion && eventoInscripcion) {
    eventoAsignacion = eventosOrdenados.find((evento) =>
      evento.tipo === 'cambio_grupo' &&
      evento.a_grupo_id &&
      (!fechaInscripcion || !iso10(evento.fecha) || iso10(evento.fecha) >= fechaInscripcion)
    ) || null
  }

  // Estudiantes anteriores al historial de eventos conservan el respaldo de
  // su grupo actual. Una venta explicita SIN grupo, en cambio, debe esperar
  // una asignacion registrada para convertirse en un inicio operativo.
  if (!eventoInscripcion && estudiante.grupo_id) {
    eventoAsignacion = {
      tipo: 'legado',
      fecha: estudiante.fecha_inscripcion,
      a_grupo_id: estudiante.grupo_id,
    }
  }

  const inicioDe = (asignacion) => {
    const g = asignacion?.a_grupo_id == null ? null : gruposPorId.get(String(asignacion.a_grupo_id)) || null
    return g ? fechaInicioOperativa(estudiante, g, asignacion) : null
  }

  // Un niño que sale de su grupo SIN haber iniciado no inició ahí: su inicio lo
  // fija la asignación siguiente (o ninguna, si quedó "Sin grupo"). No inició =
  // el cambio es anterior a su inicio, o cae en el MISMO mes de su inicio y
  // nunca tuvo un presente (ultima_asistencia es derivada; `undefined` = el
  // caller no la consultó → no se asume nada). El "mismo mes" es a propósito:
  // muchos veteranos no tienen asistencia marcada y un cambio posterior no
  // puede re-estrenarlos ni mover un mes ya pasado. Caso real: Brisas
  // sep-2026, niña sacada del grupo 99 dos días después de su inicio porque
  // arranca en octubre seguía contando como nueva de septiembre.
  if (eventoAsignacion && eventoInscripcion) {
    const nuncaAsistio = estudiante?.ultima_asistencia === null
    let fechaActual = inicioDe(eventoAsignacion)
    for (const evento of eventosOrdenados) {
      if (evento.tipo !== 'cambio_grupo' || compareEvents(evento, eventoAsignacion) <= 0) continue
      const fechaCambio = iso10(evento.fecha)
      const noInicio = !fechaActual || (fechaCambio && (fechaCambio < fechaActual ||
        (nuncaAsistio && fechaCambio.slice(0, 7) === fechaActual.slice(0, 7))))
      if (!noInicio) break
      eventoAsignacion = evento
      fechaActual = inicioDe(evento)
    }
  }

  const grupoId = eventoAsignacion?.a_grupo_id ?? null
  ...
  return {
    ...
    grupoId,
    grupo,
    fechaInicio: grupo ? fechaInicioOperativa(estudiante, grupo, eventoAsignacion) : null,
  }
}
...
export function iniciosClase(estudiantes, grupos, eventos) {
  ...
  const filas = []
  for (const estudiante of estudiantes || []) {
    if (matriculaAnulada(estudiante)) continue
...
export function estudiantesConInicioAlCierre(estudiantes, grupos, eventos, year, month) {
  if (!usaIniciosClaseOperativos(year, month)) return (estudiantes || []).filter((e) => !matriculaAnulada(e))
  const y = Number(year)
  const m = Number(month)
  const finMes = `${y}-${String(m).padStart(2, '0')}-${String(new Date(y, m, 0).getDate()).padStart(2, '0')}`
  const gruposPorId = new Map((grupos || []).map((grupo) => [String(grupo.id), grupo]))
  const inicioPorEstudiante = new Map(
    iniciosClase(estudiantes, grupos, eventos).map((inicio) => [String(inicio.estudianteId), inicio.fechaInicio]),
  )

  return (estudiantes || []).filter((estudiante) => {
    if (matriculaAnulada(estudiante)) return false
    if (!estudiante.grupo_id) {
      const fecha = iso10(estudiante.fecha_inscripcion)
      return Boolean(fecha && fecha < INICIOS_CLASE_DESDE_FECHA)
    }
    const grupo = gruposPorId.get(String(estudiante.grupo_id))
    const fechaGrupo = iso10(grupo?.fecha_inicio_clases)
    if (fechaGrupo && fechaGrupo > finMes) return false
    const fechaInicio = inicioPorEstudiante.get(String(estudiante.id))
    return !fechaInicio || fechaInicio <= finMes
  })
}
...
export function retirosActivosMes(estudiantes, grupos, eventos, year, month) {
  ...
  return (eventos || []).filter((evento) => {
    ...
    if (matriculaAnulada(estudiante)) return false
    const eventosEstudiante = eventosPorEstudiante.get(String(estudiante.id)) || []
    const { eventoInscripcion, fechaInicio } = contextoInicio(estudiante, gruposPorId, eventosEstudiante)
    ...
    if (fechaInicio) return fechaInicio <= fechaRetiro
    const fechaVenta = iso10(eventoInscripcion?.fecha) || iso10(estudiante.fecha_inscripcion)
    return Boolean(fechaVenta && fechaVenta < INICIOS_CLASE_DESDE_FECHA)
  })
}
  ...
  return {
    ...
    totales: {
      ...
      gruposActivos: grupos.filter((grupo) =>
        grupoEntraAlCuadro(grupo, finMes) && grupoOperativoConNinos(grupo, estudiantes)
      ).length,
    },
  }
}
