...
// Niños del PANEL GENERAL: manda el último mes con balance DECLARADO en el
// KPI semanal (cerrado o capturado); un mes abierto con balance vivo no mueve
// el conteo — el panel muestra los niños con los que TERMINÓ el mes anterior.
// `semilla` = cierre declarado del mes anterior al rango.
export function ninosDeclarados(months, semilla = 0) {
  const declarados = months.filter((m) => m.declarado)
  return declarados.length ? declarados[declarados.length - 1].ninosFinal : (semilla || 0)
}
