...
import { aMinutos, aHora12, DIAS_OPERATIVOS, huecosDe, coachesLibresEn } from './inventario'
...
// y los padres prefieren horarios desde las 3:30 pm — antes de esa hora es muy
// difícil vender. El sábado tiene su propia dinámica (vende desde las 9 am).
...
export function inicioVendible(dia, hueco) {
  if (esSabado(dia) || hueco.inicio >= INICIO_VENDIBLE_SEMANA) return hueco.inicio
  // Lo más tarde posible sin pasar de las 3:30 pm y dejando espacio para 1 h.
  return Math.max(hueco.inicio, Math.min(INICIO_VENDIBLE_SEMANA, hueco.fin - 60))
}
...
export function atractivoDe(grupos, retirados, dia, inicioMin) {
  ...
  // datos mandan).
  if (!esSabado(dia) && inicioMin < INICIO_VENDIBLE_SEMANA && !exacta.length) {
    score = score == null ? 3 : Math.min(score, 4)
    razon = `Entre semana antes de 3:30 pm casi no se vende: los niños salen del colegio ~1:00 pm y los padres prefieren desde las 3:30 pm.`
  }
