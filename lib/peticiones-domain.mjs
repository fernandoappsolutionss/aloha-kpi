...
const submissionMessages = Object.freeze({
  ...
  cotizacion_servicio_requerida: 'Adjunta una cotización válida en PDF del servicio y del proveedor aprobado.',
  una_cotizacion_servicio: 'Esta petición admite una cotización del servicio.',
...
export function validateServiceQuote(row, nombre) {
  return Boolean(row?.proveedor_preaprobado === true && normalizeSupplierName(nombre) &&
    normalizeSupplierName(row.proveedor_razon_social) === normalizeSupplierName(nombre) &&
    row.upload_status === 'valid' && row.archivo_sha256 && row.archivo_mime === 'application/pdf' &&
    row.archivo_bytes > 0 && row.archivo_bytes <= MAX_PDF_BYTES)
}
...
export function validateSubmission({ texto, categoria, cotizaciones, proveedor_preaprobado, proveedor_preaprobado_nombre }) {
  ...
  if (proveedor_preaprobado === true) {
    ...
    if (rows.length > 1) errors.push('una_cotizacion_servicio')
    if (!rows.some(row => validateServiceQuote(row, nombre))) errors.push('cotizacion_servicio_requerida')
    return errors
  }
