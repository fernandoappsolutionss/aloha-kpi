import { assertAdmin, assertCentroAccess, ADMIN_ROLES, esAdminDe } from './current-user.mjs'
      ...
      return {
        items: items.map((source) => {
          ...
          return {
            ...
            canAddQuote: row.tipo === 'peticion' && !row.proveedor_preaprobado && canAddQuote(row.estado),
          }
        }),
        drafts: drafts.map((source) => ({ ...presentRecord(source), expired: draftExpired(source, now()) })),
        permissions: { canChangeStatus: esAdminDe(actor, period.centroId), canDelete: ADMIN_ROLES.has(actor.rol) },
      }
    },
