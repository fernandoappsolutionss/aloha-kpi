  ...
  return {
    ...
    async changeStatus(actor, input) {
      ...
      const result = await serializable(async (query) => {
        ...
        if (row.estado === input.estado) return { ok: true, peticion: row, unchanged: true, cotizacionAprobada: null }
        // Aprobar una petición formal exige que gerencia elija cuál cotización
        // ganó; comentarios/legado y el resto de estados ignoran la selección.
        ...
        const peticion = await repo.changeStatus(query, {
          ...
          cotizacion_aprobada_id: cotizacionAprobada ? cotizacionAprobada.id : null,
        })
        await repo.insertHistory(query, {
