  ...
  return {
    ...
    async createDraft(actor, input) {
      ...
      const errors = validateSubmission({ ...input, cotizaciones: [] })
        .filter((code) => !['minimo_tres', 'nombre_proveedor_requerido'].includes(code))
    ...
    async updateDraft(actor, input) {
      ...
      return serializable(async (query) => {
        ...
        assertDraftActive(row, now())
        if (String(row.created_by) !== String(actor.id) && !ADMIN_ROLES.has(actor.rol)) throw new Error('No autorizado')
        if (input.proveedor_preaprobado === true && !row.proveedor_preaprobado) {
          const quotes = await repo.listQuotes(query, row.id, { forUpdate: true })
          if (quotes.length) throw new Error(submissionErrorMessage(['modalidad_con_cotizaciones']))
        }
        const timestamp = now()
