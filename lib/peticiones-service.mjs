import { assertAdmin, assertCentroAccess, assertWriteCentro, isMaster, puedeEscribirCentro } from './current-user.mjs'
      ...
      return {
        items: items.map((source) => {
          ...
          return {
            ...
            canAddQuote: canWrite && row.tipo === 'peticion' && canAddQuote(row.estado),
          }
        }),
        drafts: drafts.map((source) => ({ ...presentRecord(source), expired: draftExpired(source, now()) })),
        permissions: { canChangeStatus: isMaster(actor) },
      }
    },
