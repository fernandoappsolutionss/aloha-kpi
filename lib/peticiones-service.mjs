import { assertAdmin, assertCentroAccess, assertWriteCentro, isMaster, puedeEscribirCentro, esAdminDe } from './current-user.mjs'
import { DRAFT_TTL_DAYS, PETICION_ESTADOS, canAddQuote, submissionErrorMessage, validateSubmission } from './peticiones-domain.mjs'

const supplierFields = (input) => ({
  proveedor_preaprobado: input.proveedor_preaprobado === true,
  proveedor_preaprobado_nombre: input.proveedor_preaprobado === true ? String(input.proveedor_preaprobado_nombre || '').trim() : null,
})
...
const presentQuote = (quote) => ({
  ...
  proveedor_preaprobado: quote.proveedor_preaprobado === true,
...
export function createPeticionesService({
  ...
  return {
    async listPanel(actor, period) {
      ...
      const canWrite = puedeEscribirCentro(actor, period.centroId)
      ...
      return {
        items: items.map((source) => {
          ...
          return {
            ...
            canEditText: canWrite && row.tipo === 'comentario' && row.estado === 'Próximo trimestre' && String(row.created_by) === String(actor.id),
            canAddQuote: canWrite && row.tipo === 'peticion' && !row.proveedor_preaprobado && canAddQuote(row.estado),
          }
        }),
        drafts: drafts.map((source) => ({ ...presentRecord(source), expired: draftExpired(source, now()) })),
        permissions: { canChangeStatus: canWrite && esAdminDe(actor, period.centroId), canDelete: isMaster(actor) },
      }
    },

    async createComentario(actor, input) {
      assertWriteCentro(actor, input.centroId)
    ...
    async createDraft(actor, input) {
      assertWriteCentro(actor, input.centroId)
      const errors = validateSubmission({ ...input, cotizaciones: [] })
        .filter((code) => !['minimo_tres', 'nombre_proveedor_requerido', 'cotizacion_servicio_requerida'].includes(code))
    ...
    async submitPeticion(actor, input) {
      ...
      return serializable(async (query) => {
        ...
        if (String(row.created_by) !== String(actor.id) && !isMaster(actor)) throw new Error('No autorizado')
        const quotes = await repo.listQuotes(query, row.id, { forUpdate: true })
        const errors = validateSubmission({ ...row, cotizaciones: quotes })
        if (errors.length) throw new Error(submissionErrorMessage(errors))
    ...
    async changeStatus(actor, input) {
      assertWriteCentro(actor, input.centroId)
      if (!esAdminDe(actor, input.centroId)) throw new Error('No autorizado')
      ...
      const result = await serializable(async (query) => {
        ...
        let servicioAprobado = null
        if (input.estado === 'Aprobado' && row.proveedor_preaprobado) {
          const quotes = await repo.listQuotes(query, row.id, { forUpdate: true })
          const errors = validateSubmission({ ...row, cotizaciones: quotes })
          if (errors.length) throw new Error(submissionErrorMessage(errors))
          servicioAprobado = quotes[0]
        }
        ...
        if (row.estado === input.estado) {
          ...
          if (
            input.estado === 'Aprobado' && row.tipo === 'peticion' && !row.proveedor_preaprobado &&
        ...
        let cotizacionAprobada = servicioAprobado
        if (input.estado === 'Aprobado' && row.tipo === 'peticion' && !row.proveedor_preaprobado) {
        ...
        if (input.estado === 'Aprobado' && row.proveedor_preaprobado && !String(row.proveedor_preaprobado_nombre || '').trim()) {
          throw new Error('Escribe el nombre del proveedor aprobado del centro.')
        }
        const timestamp = iso(now())
    ...
    // Borrado físico desde gerencia (FODA). Una petición formal SUBMITTED
    // exige estado 'Anulada' primero (doble paso: anular, luego eliminar,
    // para no perder historial de compras por error); comentario/legado se
    // borran directo. Mismo orden que discardDraft: bloquear -> listar
    // cotizaciones con FOR UPDATE -> encolar limpieza de blobs -> borrar
    // cotizaciones -> borrar historial (RESTRICT FK) -> borrar la fila.
    async eliminarPeticion(actor, input) {
      assertAdmin(actor)
      return serializable(async (query) => {
        const row = await repo.lockPeticion(query, input.id)
        if (!row || String(row.centro_id) !== String(input.centroId)) throw new Error('Petición no encontrada.')
        if (row.tipo === 'peticion' && row.submitted_at && row.estado !== 'Anulada') {
          throw new Error('Anula la petición antes de eliminarla.')
        }
        const quotes = await repo.listQuotes(query, row.id, { forUpdate: true })
        const paths = [...new Set(quotes.flatMap((quote) => [quote.blob_pathname, quote.expected_pathname]).filter(Boolean))]
        for (const blob_pathname of paths) await repo.enqueueCleanup(query, { blob_pathname, motivo: 'peticion_eliminada' })
        await repo.deleteDraftQuotes(query, row.id)
        await repo.deleteHistorial(query, row.id)
        await repo.deletePeticionRow(query, row.id)
        return { ok: true }
      })
    },
    ...
    async updateDraft(actor, input) {
      ...
      return serializable(async (query) => {
        ...
        assertDraftActive(row, now())
        if (String(row.created_by) !== String(actor.id) && !isMaster(actor)) throw new Error('No autorizado')
        const supplier = supplierFields(input)
        if (supplier.proveedor_preaprobado !== (row.proveedor_preaprobado === true) ||
            (row.proveedor_preaprobado && supplier.proveedor_preaprobado_nombre !== String(row.proveedor_preaprobado_nombre || '').trim())) {
          const quotes = await repo.listQuotes(query, row.id, { forUpdate: true })
          if (quotes.length) throw new Error('Este borrador tiene cotizaciones. Para cambiar el proveedor o la modalidad, descártalo y crea otro.')
        }
        const timestamp = now()
