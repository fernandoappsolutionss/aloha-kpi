import { MODULO_ENCUESTAS } from './encuestas.js'
import { MODULO_PETICIONES } from './peticiones.js'
import { MODULO_CUMPLIMIENTO } from './cumplimiento.js'
...
export const MODULOS = [
  {
    ...
    quiz: [
      { pregunta: '¿Qué hace subir la barra de tu nivel?', opciones: ['Vender un niño y cobrar la inscripción completa', 'Crear la clase de prueba y llenarla de registrados', 'Que el niño empiece clases', 'Cerrar el mes con el KPI en Coincide'], explicacion: 'El nivel se mide por niños activos, y un niño es activo desde su fecha de inicio de clases. La venta sola no mueve la barra.' },
      ...
      { pregunta: 'La Ruta de Nivel dice "Sin fecha confiable". ¿Qué significa?', opciones: ['Que vas mal y no llegas al nivel este trimestre', 'Que el sistema está caído y hay que esperar', 'Que ya llegaste a la meta', 'Que faltan fechas de inicio por cargar'], explicacion: 'Sin fechas de inicio el sistema no puede proyectar. Carga las fechas y la ruta se vuelve confiable.' },
    ],
    errores: [
  ...
  },
  {
    ...
    quiz: [
      { pregunta: '¿Qué es lo único que capturas a mano en el sistema?', opciones: ['Los royalties que el centro le paga a la franquicia cada mes', 'La realidad operativa: grupos, niños, retiros', 'El KPI mensual completo: nuevos, retiros, activos y deserción', 'El promedio de niños por grupo'], explicacion: 'Los números los produce el sistema a partir de los hechos operativos que tú registras: grupos, clases de prueba, inscripciones, retiros.' },
      { pregunta: 'El Cuadro muestra 0 nuevos activos aunque vendiste 5. ¿Qué haces?', opciones: ['Editar el número en el Cuadro para que cuadre con lo que vendiste', 'Cerrar el mes y seguir: al siguiente se corrige solo', 'Revisar la fecha de inicio del grupo y el grupo de cada niño', 'Borrar los niños y volverlos a meter con la fecha de hoy'], explicacion: 'Un número forzado descuadra al mes siguiente. Se corrige el hecho de origen y el Cuadro se recalcula.' },
      { pregunta: '¿Cuál es el orden correcto del modelo?', opciones: ['Niño inscrito → grupo → clase de prueba para llenarlo', 'Grupo → clase de prueba amarrada → niño en el grupo', 'Clase de prueba → niño inscrito → grupo que lo reciba', 'Da igual el orden: el sistema lo acomoda después'], explicacion: 'Primero existe el grupo; la clase de prueba lo llena; el niño entra a ese grupo.' },
    ],
    errores: [
  ...
  },
  {
    ...
    quiz: [
      { pregunta: '¿Cuál es el campo más importante al aperturar un grupo?', opciones: ['Número de grupo, para que el centro lo reconozca', 'Niños con los que abre, por el mínimo del manual', 'Grupo online, por la exención de fusión', 'Fecha de inicio de clases'], explicacion: 'Esa fecha mete al grupo en el Cuadro y convierte a sus niños en activos. Sin ella, no hay nivel que suba.' },
      { pregunta: 'Abres un TINY nivel 1 con 5 niños. ¿Qué pasa?', opciones: ['Abre, pero bajo responsabilidad del centro', 'El sistema no lo deja abrir hasta que llegues a 8', 'Se marca como online y sale del promedio por grupo', 'Se fusiona solo con otro TINY'], explicacion: 'El sistema avisa el mínimo del manual (8 para TINY) pero no bloquea: la responsabilidad queda en el centro.' },
      { pregunta: 'Llega la fecha de inicio del grupo. ¿Qué sigues pudiendo editar?', opciones: ['Todo, mientras el mes no esté cerrado', 'Nada: el grupo queda congelado', 'Solo horario y coach', 'Solo el número y el itinerario'], explicacion: 'Cuando el grupo inició, queda cerrado a edición salvo horario y coach.' },
    ],
    errores: [
  ...
  },
  {
    ...
    quiz: [
      { pregunta: '¿Para qué sirve el campo "Grupo que se va a aperturar"?', opciones: ['Para ponerle nombre a la clase en el calendario', 'Para calcular el precio de la inscripción', 'Para amarrar la clase al grupo que estás llenando', 'Para fijar la zona horaria de los recordatorios automáticos'], explicacion: 'Es el amarre del modelo: sin él, la clase de prueba queda suelta y ventas no ve los cupos.' },
      { pregunta: 'Ves "Sin grupo relacionado" en la lista. ¿Qué significa?', opciones: ['La clase se canceló y ya no recibe registrados', 'Se creó sin amarre a un grupo', 'No hubo registrados y el sistema la archivó', 'Es una clase gratis, sin grupo porque no vende'], explicacion: 'Nadie sabe qué grupo llenaba. Se corrige editando la clase y eligiendo el grupo.' },
      { pregunta: '¿Dónde ves si tu clase de prueba sirvió?', opciones: ['En el FODA, en la fila de oportunidades', 'En la Ruta de Nivel, en los escenarios', 'En el Cuadro, en nuevos activos', 'En el tablero de Clases de Prueba, arriba'], explicacion: 'Ese tablero es tu embudo: registrados → asistieron → pagados.' },
    ],
    errores: [
      ...
      { sintoma: 'No veo las clases del mes pasado.', causa: 'El periodo abre en Este mes.', arreglo: 'En Clases de Prueba selecciona Mes pasado o Elegir mes. Las tarjetas muestran el periodo y el filtro Todas, Realizadas o Próximas que elijas.' },
      { sintoma: 'Quiero comparar Clases de Prueba con el KPI.', causa: 'Debes comparar las mismas clases realizadas del mes.', arreglo: 'En KPI pulsa Ver clases realizadas de este mes. Con el filtro Realizadas, Invitados coincide con Registrados y Asistieron con Asistieron; los registros de clases futuras quedan en Próximas. Matriculados cuenta los registros con negocio ganado en esas mismas clases; se corrige en el CRM. Pagados refleja los pagos registrados. Un KPI cerrado conserva la foto del cierre.' },
    ],
  },
  {
    id: 'inscribir',
    ...
    quiz: [
      { pregunta: 'Inscribes un niño con Grupo = "Sin grupo". ¿Qué pasa?', opciones: ['Cuenta como activo igual desde la fecha de inscripción', 'Se asigna solo al grupo más vacío de su itinerario', 'Queda en niños sin grupo y no cuenta como activo', 'Se borra a los 7 días si nadie lo asigna'], explicacion: 'Sin grupo no hay fecha de inicio, y sin fecha de inicio no hay niño activo: cuenta cuando lo asignes.' },
      { pregunta: '¿Qué fecha va en "Fecha de inscripción"?', opciones: ['La del inicio de clases del grupo', 'La de la clase de prueba a la que asistió', 'La del cierre de nivel que le toca', 'La del día que pagó'], explicacion: 'El inicio de clases lo da el grupo; la inscripción es cuando el representante pagó.' },
      { pregunta: '"Cierre de nivel (override)": ¿cuándo se llena?', opciones: ['Siempre, al inscribir', 'Solo si ese niño va a otro ritmo que el grupo', 'Nunca: lo calcula el sistema', 'Solo para Kinder'], explicacion: 'Por defecto el cierre se deriva del plan del grupo. El override es la excepción.' },
    ],
    errores: [
      { sintoma: 'Una familia pide devolución antes de iniciar clases.', causa: 'La inscripción debe anularse antes del inicio, para que no figure como ingreso ni deserción.', arreglo: 'Usa Anular matrícula y registra el motivo y la devolución. La ficha conserva su historial. Si ya inició, corresponde Retirado; una ficha duplicada se elimina o fusiona aparte. Los meses cerrados requieren revisión histórica.' },
  ...
  },
  {
    ...
    quiz: [
      { pregunta: 'El botón "Inscribir niño aquí" está gris. ¿Qué pasó?', opciones: ['Venció la ventana de niños nuevos del manual', 'Un error del sistema: recarga la página', 'El grupo no tiene coach asignada todavía', 'El mes está cerrado y no admite cambios'], explicacion: 'TINY hasta la semana 4, KIDS hasta la 2. El botón gris es el manual aplicado.' },
      { pregunta: 'Vendiste un niño y el grupo está cerrado a nuevos. ¿Qué haces?', opciones: ['Le pides a gerencia que lo meta igual por esta vez', 'Le borras la fecha de inicio al grupo para que vuelva a abrir', 'Extender ventana si alcanza al grupo, o al próximo grupo', 'Le cambias el itinerario al grupo para ganar semanas'], explicacion: 'Son las dos salidas legítimas. Forzarlo condena al niño a ir perdido.' },
      { pregunta: '¿Qué hace la palanca "Cerrar inscripciones"?', opciones: ['Bloquea solo a los niños nuevos, igual que la ventana automática', 'Cierra el mes del grupo en el KPI', 'Borra el grupo y manda a sus niños a sin grupo', 'Bloquea a todos: nuevos, traslados y reincorporaciones'], explicacion: 'Es la palanca manual fuerte. La ventana automática solo frena a los nuevos.' },
    ],
    errores: [
  ...
  },
  {
    ...
    duracionMin: 4,
    audioActualizado: true,
    intro: {
      texto: 'Cada niño tiene su propio plan, según el nivel que cursa y cuándo lo empezó. Al fusionar grupos, pueden compartir coach y horario y seguir en semanas distintas. En Itinerario verás primero el avance de los niños. La referencia del aula aparece después.',
      voz: 'Cada niño tiene su propio plan. <break time="0.4s"/> Según el nivel que cursa, y cuándo lo empezó. <break time="0.5s"/> Al fusionar grupos, pueden compartir coach y horario, <break time="0.3s"/> y seguir en semanas distintas. <break time="0.5s"/> En Itinerario verás primero el avance de los niños. <break time="0.4s"/> La referencia del aula aparece después.',
    },
    inicio: { ruta: '/centro/{id}/grupos' },
    pasos: [
      { id: 'it-1', tipo: 'hazlo', target: 'grupos.tarjeta', titulo: 'Abre un grupo', texto: 'Haz clic en un grupo para abrir su panel. Puede tener niños de distintos niveles o que empezaron en fechas diferentes.' },
      { id: 'it-2', tipo: 'hazlo', target: 'grupo.tab-itinerario', titulo: 'Pestaña Itinerario', texto: 'Abre Itinerario. Puedes consultar y completar los planes individuales aunque falte la referencia general del aula.' },
      { id: 'it-3', tipo: 'mostrar', target: 'grupo.planes-ninos', titulo: 'Avance de los niños', texto: 'Aquí ves qué nivel y semana cursa cada niño. Si falta su fecha, usa Completar inicio. Es el comienzo de su nivel actual. Si varios empezaron juntos, puedes usar Todos empezaron el mismo día; confírmalo con la coach.' },
      { id: 'it-4', tipo: 'mostrar', target: 'grupo.ajustar-itinerario', titulo: 'Referencia del aula', texto: 'Crear la referencia no completa los inicios de los niños. Al ajustar el nivel o la fecha de una referencia existente, el formulario indica a qué niños afecta. Las suspensiones se aplican al calendario del aula y pueden mover los cierres individuales.' },
      ...
      { id: 'it-6', tipo: 'mostrar', target: 'grupo.panel', titulo: 'Cuando falta información', texto: 'Si un niño no tiene inicio de nivel, consulta la planificación de la coach. No uses la fecha de fusión ni la de hoy para llenar el campo. Si ya tiene inicio pero no aparece su semana, revisa el horario del grupo.' },
    ],
    quiz: [
      { pregunta: 'Dos niños del mismo grupo van por semanas distintas. ¿Qué haces?', opciones: ['Les asignas la fecha de la fusión', 'Pones la fecha del niño más avanzado a todos', 'Conservas el inicio y el avance de cada niño', 'Reinicias el nivel de ambos desde hoy'], explicacion: 'Compartir grupo y horario no significa haber comenzado el nivel juntos. Cada niño conserva su planificación.' },
      { pregunta: '¿Cuándo usas Todos empezaron el mismo día?', opciones: ['Cuando la coach confirma que esos niños comenzaron su nivel juntos', 'Siempre que estén en el mismo grupo', 'Cuando no conoces ninguna fecha', 'Cuando quieres terminar más rápido'], explicacion: 'Solo aplica una fecha común cuando corresponde a todos. Si empezaron en fechas distintas, completa cada inicio por separado.' },
      { pregunta: '¿Con quién compartes el link del coach?', opciones: ['En el grupo de WhatsApp del centro', 'Con los representantes del grupo', 'Con la junta', 'Solo con la coach de ese grupo'], explicacion: 'Es un enlace privado por grupo que da acceso sin clave: quien lo tenga puede marcar asistencia.' },
    ],
    errores: [
      { sintoma: 'No cargué el itinerario porque el grupo tiene inicios distintos.', causa: 'Se confundió la referencia del aula con los planes individuales.', arreglo: 'Abre Itinerario y completa el inicio de cada niño. Puedes hacerlo aunque falte la referencia general.' },
      { sintoma: 'Sé por qué semana va el niño, pero no cuándo empezó el nivel.', causa: 'Falta confirmar la fecha histórica.', arreglo: 'Consulta el cuaderno de la coach antes de guardar. No inventes una fecha para completar el campo.' },
      { sintoma: 'El niño tiene inicio de nivel, pero no aparece su semana.', causa: 'Puede faltar el calendario de clases.', arreglo: 'Revisa el horario del grupo y conserva la fecha del niño.' },
    ],
  },
  {
    id: 'fusiones',
    ...
    quiz: [
      { pregunta: '¿Quiénes quedan fuera de toda fusión?', opciones: ['Los de nivel 3 o más', 'Los Kinder y los de nivel 1-2', 'Los de grupos de sábado', 'Todos se fusionan'], explicacion: 'Nivel 1-2 está aprendiendo la base; Kinder queda fuera de toda fusión.' },
      ...
      { pregunta: 'Antes de "Aplicar fusión" debes…', opciones: ['Cerrar el mes para que el movimiento quede registrado', 'Hablar con la coach y los representantes', 'Borrar el grupo de origen para liberar el horario', 'Nada: el sistema avisa a todos por ti'], explicacion: 'El sistema mueve niños; la conversación la haces tú, o se te caen en el camino.' },
    ],
    errores: [
  ...
  },
  {
    ...
    pasos: [
      ...
      { id: 'ci-3', tipo: 'hazlo', target: 'nav.kpi', titulo: 'Vamos al KPI', texto: 'Haz clic en KPI Mensual en el menú.', voz: 'Haz clic en KPI Mensual, <break time="0.3s"/> en el menú.', audioActualizado: true, vozLegacy: 'Haz clic en KPI Semanal en el menú.', ruta: '/centro/{id}/kpi' },
    ...
    quiz: [
      { pregunta: '¿Qué revisas antes de cerrar el mes?', opciones: ['Solo los royalties del mes que cierras', 'Cuadro vs KPI en Coincide, retiros y fechas de inicio', 'El FODA y las peticiones pendientes de todo el trimestre', 'Nada: el cierre revisa y corrige todo por ti'], explicacion: 'El cierre congela lo que haya. Si hay descuadre, queda en el historial.' },
      { pregunta: 'Al cerrar, ¿qué hace el sistema con los retiros programados que vencían en el mes?', opciones: ['Los borra y deja al niño como activo', 'Los pasa al mes siguiente para que los revises', 'Los ejecuta y los cuenta en la deserción', 'No hace nada: eso lo hace el cron de la noche'], explicacion: 'El cierre no se traga la deserción: la aplica él mismo si el cron no lo hizo.' },
      { pregunta: 'Reabres junio para corregir y julio ya estaba cerrado. ¿Qué pasa con julio?', opciones: ['Se actualiza solo con lo que corrijas en junio', 'Se borra y hay que digitarlo otra vez desde cero', 'No pasa nada: cada mes es independiente', 'Hay que reabrirlo y cerrarlo de nuevo'], explicacion: 'Cada mes arranca con el cierre del anterior; la cadena la rehaces tú, en orden.' },
    ],
    errores: [
...
export const ERRORES_GLOBALES = [
  { modulo: 'inscribir', sintoma: 'Inscribir "para asignar el grupo luego".', causa: 'El niño queda sin grupo y no cuenta como activo.', arreglo: 'Si ya sabes el grupo, ponlo al inscribir. Si no, entra en Grupos y Fusiones → Sin grupo y retirados → Asignar grupo apenas lo sepas.' },
