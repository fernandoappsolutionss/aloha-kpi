...
//
// `centros` (opcional en el módulo) lo limita a esos centros: la materia fiscal
// de Venezuela es solo de Los Naranjos. Sin `centroId` el módulo con `centros`
// queda FUERA (fail-closed): quien no dice de qué centro es no lo recibe.
export const alcanzaCentro = (m, centroId) =>
  !m?.centros || (centroId != null && m.centros.includes(Number(centroId)))

// ¿Este módulo es del plan de esta persona? El mismo criterio que planDeRol,
// para las puertas que miran un módulo suelto.
export const esDelPlan = (m, rol, centroId) =>
  (m?.roles || []).includes(rol) && alcanzaCentro(m, centroId)

export function planDeRol(rol, modulos, centroId) {
  return (modulos || [])
    .filter((m) => esDelPlan(m, rol, centroId))
    .slice()
...
export const OFICIAL_DE = {
  asistente: ['administradora', 'coordinador', 'admin_master'],
  administradora: ['coordinador', 'admin_master'],
  coach: ['administradora', 'coordinador', 'admin_master'],
  coordinador: ['admin_master'],
}
...
export function puedeFirmar(firmante, alumno) {
  ...
  if (firmante.rol === 'admin_master') return true
