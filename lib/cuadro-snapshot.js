...
import { iniciosClaseMes, usaIniciosClaseOperativos } from './inicios-clase.mjs'
...
export async function calcularCuadro(centroId, y, m) {
  const todos = await armarGrupos(centroId)
  const todosEst = await sql`SELECT * FROM estudiantes WHERE centro_id = ${centroId}`
  ...
  const nuevosActivosIds = usaIniciosOperativos
    ...
    : null
  const controlCalculado = cuadroControlGrupos(grupos, estudiantes, eventos, nuevosActivosIds)
  ...
  return {
    royalties: cuadroRoyalties(estudiantes, eventos, royaltyRate, nuevosActivosIds),
    controlGrupos,
