...
import { INICIOS_CLASE_DESDE_FECHA, iniciosClaseMes, retirosActivosMes, usaIniciosClaseOperativos } from './inicios-clase.mjs'
...
export async function calcularCuadro(centroId, y, m, query = sql) {
  ...
  const estudiantes = todosEst.filter((e) => {
    if (usaIniciosOperativos && !e.grupo_id) {
      return Boolean(fechaIso10(e.fecha_inscripcion) && fechaIso10(e.fecha_inscripcion) < INICIOS_CLASE_DESDE_FECHA)
    }
  ...
  const todosEventos = await query`
    ...
    WHERE centro_id = ${centroId}
      AND ((year = ${y} AND month = ${m}) OR tipo IN ('inscripcion', 'retiro', 'cambio_grupo'))
    ORDER BY fecha, id
  `
