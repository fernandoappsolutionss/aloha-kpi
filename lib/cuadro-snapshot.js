...
import { iniciosClaseMes, retirosActivosMes, usaIniciosClaseOperativos, grupoEntraAlCuadro } from './inicios-clase.mjs'
...
export async function calcularCuadro(centroId, y, m, query = sql) {
  ...
  // Los eventos de membresía (inscripcion/retiro/reincorporacion) vienen
  // COMPLETOS, de todos los meses: la población as-of (g1-22) necesita el
  // histórico entero para rebobinar un estado reescrito por eventos futuros.
  const todosEventos = await query`
    SELECT * FROM estudiante_eventos
    WHERE centro_id = ${centroId}
      AND ((year = ${y} AND month = ${m}) OR tipo IN ('inscripcion', 'retiro', 'reincorporacion'))
    ORDER BY fecha, id
  `
  // Población "as of" fin de mes (R5, g1-22): en un mes del gate operativo la
  // selección deja de mirar `estudiantes.estado` de hoy y proyecta el estado
  // al cierre del mes desde eventos — el retiro programado que el cron ejecutó
  // el 1-sep (evento year/month = septiembre) no borra al niño de agosto
  // abierto. Sin eventos posteriores al mes la proyección ES el estado actual
  // (paridad exacta, regresión cero). Pre-gate (< 202608) el modo legacy sigue
  // intacto. La foto de un mes CERRADO no pasa por aquí: se lee del snapshot.
  const usaIniciosOperativos = usaIniciosClaseOperativos(y, m)
  const estudiantes = usaIniciosOperativos ? poblacionAsOfMes(visibles, todosEventos, y, m) : visibles
