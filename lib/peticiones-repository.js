...
export const peticionesRepository = {
  ...
  // cotizacion_aprobada_id: COALESCE contra la propia columna deja que un
  // valor NULL/undefined en data (cualquier transición que no sea Aprobado
  // sobre tipo 'peticion') conserve lo que ya estaba guardado, en vez de
  // borrarlo en cada cambio de estado.
  async changeStatus(query, data) {
    const [row] = await query(
      `UPDATE peticiones
       SET estado = $2, anulada_at = $3, cotizacion_aprobada_id = COALESCE($4, cotizacion_aprobada_id), updated_at = now()
       WHERE id = $1
       RETURNING *`,
