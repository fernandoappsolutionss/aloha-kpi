...
export const peticionesRepository = {
  ...
  // descargable a partir de SOLO su id. El centro_id se deriva del JOIN con
  // peticiones, nunca lo aporta quien llama; solo expone las columnas que la
  // descarga necesita y únicamente cuando upload_status = 'valid' (un intento
  // pending/validating/invalid nunca es descargable).
  async findDownloadableQuote(cotizacionId) {
    const [row] = await sql`
      SELECT c.id, p.centro_id AS centro_id, c.archivo_nombre, c.blob_pathname
      FROM peticion_cotizaciones c
      JOIN peticiones p ON p.id = c.peticion_id
      WHERE c.id = ${cotizacionId} AND c.upload_status = 'valid'
    `
    return row || null
  },
}
