...
import { ADMIN_ROLES } from './current-user.mjs'
...
const QUOTES_SUBQUERY = `
  COALESCE((
    SELECT json_agg(json_build_object(
      ...
      'proveedor_preaprobado', c.proveedor_preaprobado,
...
export const peticionesRepository = {
  ...
  // Borradores (submitted_at IS NULL) del centro/periodo. admin_general/
  // supervisor (ADMIN_ROLES) ven todos los del centro; cualquier otro rol
  // -incluido uno desconocido o futuro- cae en la rama restringida (solo
  // los suyos), nunca al revés.
  ...
  async insertDraft(query, data) {
    const [row] = await query(
      `INSERT INTO peticiones
         (centro_id, anio, trimestre, texto, tipo, categoria, estado, created_by, created_by_snapshot, submitted_at, draft_expires_at, proveedor_preaprobado, proveedor_preaprobado_nombre)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9::jsonb, $10, $11, $12, $13)
       RETURNING *`,
      [
        data.centro_id, data.anio, data.trimestre, data.texto, data.tipo, data.categoria,
        data.estado, data.created_by, JSON.stringify(data.created_by_snapshot), data.submitted_at, data.draft_expires_at,
        data.proveedor_preaprobado === true, data.proveedor_preaprobado_nombre ?? null,
      ]
    )
    return row
  },
  ...
  async updateDraft(query, data) {
    const [row] = await query(
      `UPDATE peticiones
       SET texto = $2, categoria = $3, draft_expires_at = $4,
           proveedor_preaprobado = $5, proveedor_preaprobado_nombre = $6, updated_at = now()
       WHERE id = $1 AND submitted_at IS NULL
       RETURNING *`,
      [data.id, data.texto, data.categoria, data.draft_expires_at, data.proveedor_preaprobado === true, data.proveedor_preaprobado_nombre ?? null]
    )
    return row
  },
  ...
  // cotizacion_aprobada_id se fija SIEMPRE al valor que manda el servicio
  // (nunca COALESCE contra lo guardado): si la petición sale de 'Aprobado'
  // hacia cualquier otro estado, la ganadora anterior debe borrarse, no
  // sobrevivir a una negación/anulación/reapertura posterior.
  async changeStatus(query, data) {
    const [row] = await query(
      `UPDATE peticiones
       SET estado = $2, anulada_at = $3, cotizacion_aprobada_id = $4, updated_at = now()
       WHERE id = $1
       RETURNING *`,
      [data.id, data.estado, data.anulada_at, data.cotizacion_aprobada_id ?? null]
    )
    return row
  },

  // Corrige/backfillea SOLO la cotización ganadora de una petición que ya
  // está Aprobada (el estado no cambia, así que no hay evento de historial
  // que insertar; ver lib/peticiones-service.mjs:changeStatus).
  async setApprovedQuote(query, { id, cotizacionAprobadaId }) {
    const [row] = await query(
      `UPDATE peticiones
       SET cotizacion_aprobada_id = $2, updated_at = now()
       WHERE id = $1
       RETURNING *`,
      [id, cotizacionAprobadaId]
    )
    return row
  },
  ...
  // Borrado físico (gerencia, FODA): limpia el historial de estados ANTES de
  // borrar la petición (FK ON DELETE RESTRICT lo exige).
  async deleteHistorial(query, peticionId) {
    await query('DELETE FROM peticion_estado_historial WHERE peticion_id = $1', [peticionId])
  },

  // Borrado físico (gerencia, FODA): borra la fila sin la guarda
  // "submitted_at IS NULL" de deleteDraft — aquí se permite eliminar
  // peticiones ya enviadas (tras anularlas) además de comentarios/legado.
  async deletePeticionRow(query, peticionId) {
    const [row] = await query('DELETE FROM peticiones WHERE id = $1 RETURNING id', [peticionId])
    return row
  },
      ...
      const [inserted] = await query(
        `INSERT INTO peticion_cotizaciones
           (peticion_id, proveedor_razon_social, proveedor_clave, proveedor_pais, proveedor_id_fiscal,
            ...
            upload_status, upload_attempts, validation_error, uploaded_by, uploaded_by_snapshot, validada_at, proveedor_preaprobado)
        ...
        [
          ...
          row.upload_status, row.upload_attempts, row.validation_error, row.uploaded_by, JSON.stringify(row.uploaded_by_snapshot), row.validada_at, row.proveedor_preaprobado === true,
        ]
      )
      return inserted
    }
    const [updated] = await query(
      `UPDATE peticion_cotizaciones
       SET proveedor_razon_social = $2, proveedor_clave = $3, proveedor_pais = $4, proveedor_id_fiscal = $5,
           ...
           uploaded_by = $19, uploaded_by_snapshot = $20::jsonb, validada_at = $21, proveedor_preaprobado = $22, updated_at = now()
       WHERE id = $1
       RETURNING *`,
      [
  ...
  async getCallbackContext(payload, query, { lockUser = false } = {}) {
    ...
    const text = `
      SELECT c.*, p.centro_id AS centro_id, p.created_by AS created_by, p.submitted_at AS submitted_at, p.estado AS estado,
             (u.id IS NOT NULL) AS user_exists, u.password_hash AS user_password_hash, u.email AS user_email,
             u.rol AS user_rol, u.centro_id AS user_centro_id, u.blocked_until AS user_blocked_until,
             (u.blocked_until IS NOT NULL AND u.blocked_until > CURRENT_TIMESTAMP) AS user_bloqueado,
             COALESCE(ARRAY_AGG(uc.centro_id) FILTER (WHERE uc.centro_id IS NOT NULL), '{}') AS user_centros
      FROM peticion_cotizaciones c
      JOIN peticiones p ON p.id = c.peticion_id
  ...
  async expireDrafts(now) {
    return await withTransaction(async (query) => {
      ...
      for (const draft of drafts) {
        const quotes = await peticionesRepository.listQuotes(query, draft.id, { forUpdate: true })
  ...
  async reconcileBlobPage(now, { checkpoint, listedBlobs, staleMinutes, expectedCursor, nextCursor }) {
    return await withTransaction(async (query) => {
      let claimed = await query('SELECT cursor FROM peticion_cleanup_checkpoint WHERE checkpoint_key = $1 FOR UPDATE', [checkpoint])
      if (!claimed.length) {
        // Fila semilla ausente (seed saltado o borrada a mano): la creamos
        // en la misma transacción y volvemos a leerla con FOR UPDATE, para
        // no dejar el UPDATE final huérfano (0 filas afectadas y re-scan
        // infinito de la primera página reportando éxito en silencio).
        await query(
        ...
        claimed = await query('SELECT cursor FROM peticion_cleanup_checkpoint WHERE checkpoint_key = $1 FOR UPDATE', [checkpoint])
      }
      const [row] = claimed
  ...
  // No transaccional: destinatarios del correo de decisión (aprobado/negado)
  // para un centro — solo usuarios con cuenta activa (password_hash) y correo
  // registrado, sin importar su rol.
  async listCentroRecipients(centroId) {
    return await sql`
      SELECT nombre, email FROM usuarios
      WHERE centro_id = ${centroId} AND password_hash IS NOT NULL AND email IS NOT NULL
    `
  },
}
