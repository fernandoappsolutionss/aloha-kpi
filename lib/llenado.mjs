...
// Regla de Fernando 2026-08-10: la ventana de niños nuevos es del LLENADO del
// grupo, no de cada nivel. El itinerario se regenera al subir de nivel, así que
// sin esta guarda un grupo veterano volvía a "en llenado" cada vez que empezaba
// un nivel (Calle 50 tenía a G22 en nivel 10 aceptando nuevos). Del nivel 2 en
// adelante el grupo queda CERRADO a nuevos, y la palanca abierta NO lo reabre:
// la única salida es `llenado_extendido_hasta` (extenderVentanaLlenado), que es
// una decisión consciente, queda con rastro y vence sola. Los movimientos
// internos (reincorporación, cambio de grupo, fusión) nunca pasan por aquí.
export function fechaLimiteNuevos(grupo) {
  ...
  const nivel = Number(it?.nivel)
  if (Number.isFinite(nivel) && nivel > 1) {
    // La extensión manual sigue mandando: si un admin extendió la ventana de
    // este grupo a propósito, se respeta hasta su fecha.
    const extensionNivel = iso10(grupo?.llenado_extendido_hasta)
    if (extensionNivel) return { fechaLimite: extensionNivel, razon: null, extendida: true }
    return { fechaLimite: null, razon: 'nivel_avanzado', extendida: false }
  }
...
export function ventanaNuevos(grupo, hoyIso) {
  ...
  // Nivel 2+ sin extensión manual: cerrado de plano (no es 'sin límite').
  const cerradoPorNivel = razonLimite === 'nivel_avanzado'
  // Sin días calculables (exento o sin hoy) se falla ABIERTO: nunca cerrar a ciegas.
  const vigente = !cerradoPorNivel && (sinLimite || diasHastaLimite == null || diasHastaLimite >= 0)

  let razon
  if (!activo) razon = 'no_activo'
  else if (!palanca) razon = 'palanca_cerrada'
  else if (cerradoPorNivel) razon = 'nivel_avanzado'
...
export function ritmoLlenado(grupo, meta, eventosInscripcion, hoyIso) {
  ...
  // Edad del grupo desde su PUBLICACIÓN: día civil Panamá de created_at
  // (g1-3). fecha_apertura es columna congelada y ya no participa.
  const creado = fechaPublicacion(grupo?.created_at)
