...
export const usuariosRepository = {
  ...
  // Si la ficha ya existe (la crea la administradora en Grupos y Fusiones y de
  // ella cuelgan los grupos), se escoge del desplegable en vez de volver a
  // teclear el nombre. Si es gente nueva, la ficha NACE con la cuenta
  // (insertCoach) — dar de alta un coach no puede exigir registrarlo dos veces.
  // En los dos caminos la cuenta y el horario quedan pegados por
  // coaches.usuario_id.
  ...
  // Un mismo nombre dos veces en el centro serían dos fichas para una persona
  // y los grupos repartidos entre ambas. Se compara sin may/min ni espacios de
  // sobra, y también contra las desactivadas: reactivar es de la
  // administradora, no de un alta de cuenta.
  async findCoachByName(query, centroId, nombre) {
    const [coach] = await query(
      `SELECT id, centro_id, nombre, activo, usuario_id FROM coaches
       WHERE centro_id = $1 AND LOWER(BTRIM(nombre)) = LOWER(BTRIM($2))`,
      [Number(centroId), String(nombre)]
    )
    return coach
  },

  async insertCoach(query, { centro_id, nombre }) {
    const [coach] = await query(
      `INSERT INTO coaches (centro_id, nombre) VALUES ($1, $2)
       RETURNING id, centro_id, nombre, activo, usuario_id`,
      [Number(centro_id), String(nombre).trim()]
    )
    return coach
  },
