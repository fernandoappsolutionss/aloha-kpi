...
import { iniciosClaseMes } from './inicios-clase.mjs'
...
export function decidirFechaInicio({ itInicio, universo }) {
  const inicio = iso10(itInicio)
  if (!inicio) return { fecha: null, regla: 'sin_it_inicio', minInscripcion: null }
  const filas = universo || []
  if (!filas.length) return { fecha: inicio, regla: 'sin_ninos', minInscripcion: null }
  if (filas.some((fila) => !fila.fecha)) {
    return { fecha: null, regla: 'universo_sin_fecha', minInscripcion: null }
  }
  const minInscripcion = filas.reduce((min, fila) => (fila.fecha < min ? fila.fecha : min), filas[0].fecha)
  return {
    fecha: minInscripcion < inicio ? minInscripcion : inicio,
    regla: 'con_ninos',
    minInscripcion,
  }
}
...
export function comparableMes({ grupos, estudiantes, eventos, year, month, royaltyRate }) {
  ...
  const iniciado = (g) => !g.fecha_inicio_clases || iso10(g.fecha_inicio_clases) <= finMes
...
// documentada y aprobada: un grupo VACÍO (regla 'sin_ninos') con it_inicio
// futuro sale del cuadro de los meses previos a su fecha (semántica "entra al
// cuadro ese mes"). Esa variación se lista APARTE como cambio intencional y no
// cuenta como diferencia. `decisiones` es Map(String(grupoId) → decisión).
export function compararComparables(antes, despues, { decisiones, finMes }) {
  ...
  for (const id of antes.gruposIds) {
    ...
    if (decision?.regla === 'sin_ninos' && decision.fecha && decision.fecha > finMes) {
