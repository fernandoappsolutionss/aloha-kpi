// Backfill D7 de grupos.fecha_inicio_clases — lógica PURA, sin BD.
//
// Regla de Fernando 2026-08-08 (R1): TODO grupo lleva la fecha de inicio de
// clases de su itinerario (`itinerario_clases.fecha_inicio`); el que no tiene
// itinerario queda vacío hasta que se le cree. Como `fechaInicioOperativa`
// (lib/inicios-clase.mjs) es max(inscripción, fecha del grupo) SIN excepciones
// por nivel, un grupo veterano cuya fecha quede en el arranque FUTURO de su
// nivel vigente reclasificaría a su gente: ese caso lo corrige el PREFLIGHT de
// datos del rollout (g1-1: LEAST(fecha asignada, min inscripción atribuida)) y
// la verificación before/after de este módulo lo detecta y ABORTA.
//
...
import { iniciosClaseMes, grupoEntraAlCuadro } from './inicios-clase.mjs'
...
// OJO (R1, g1-1): grupoYaDioClases murió — ya no hay excepción por nivel que
// proteja a los veteranos. La garantía de cero reclasificación vive en el
// PREFLIGHT de datos del rollout: todo grupo veterano cuya fecha quedó en el
// arranque FUTURO de su nivel vigente se corrige a
// LEAST(fecha asignada, min(fecha_inscripcion de sus niños atribuidos)) antes
// de activar R1. La verificación before/after de este módulo es la que detecta
// (y aborta) cualquier caso que ese preflight deba resolver.
...
export function comparableMes({ grupos, estudiantes, eventos, year, month, royaltyRate }) {
  ...
  // Misma regla que lib/cuadro-snapshot.js (R1, g1-1): el cuadro depende de
  // fecha_inicio_clases con la MISMA guardia de runtime (veterano nivel 2+
  // con fecha futura cuenta como iniciado) — el comparable debe reproducir
  // exactamente lo que el cuadro vivo hace con los mismos datos.
