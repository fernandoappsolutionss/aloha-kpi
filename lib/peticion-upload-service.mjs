...
const callbackActor = (row, payload) => ({
  id: Number(payload.uid),
  email: row?.user_email,
  rol: row?.user_rol,
  centro_id: row?.user_centro_id,
  centros: (row?.user_centros || []).map(Number),
  password_hash: row?.user_password_hash,
  blocked_until: row?.user_blocked_until,
  bloqueado: row?.user_bloqueado,
})
const activeCallbackActor = (row, payload) => Boolean(
  row?.user_exists
  && row?.user_password_hash
  && !usuarioBloqueado(callbackActor(row, payload))
  && puedeEscribirCentro(callbackActor(row, payload), row.centro_id)
  && (row.submitted_at || String(row.created_by) === String(payload.uid) || isMaster(callbackActor(row, payload)))
)
...
const assertQuoteWriteAccess = (actor, row) => {
  assertWriteCentro(actor, row.centro_id)
  if (!row.submitted_at && String(row.created_by) !== String(actor.id) && !isMaster(actor)) {
    throw new Error('Carga no autorizada.')
  }
}
...
export function createPeticionUploadService({ repo, blob, now = () => new Date(), uuid = () => crypto.randomUUID() }) {
  ...
  return {
    async prepare(actor, input) {
      try {
        return await repo.transaction(async (query) => {
          ...
          assertWriteCentro(actor, petition.centro_id)
          const preapproved = petition.proveedor_preaprobado === true
          if (preapproved && petition.submitted_at) throw new Error('La cotización de una petición enviada no se puede cambiar.')
          if (!petition.submitted_at && String(petition.created_by) !== String(actor.id) && !isMaster(actor)) throw new Error('No autorizado')
          ...
          if (petition.submitted_at && (petition.proveedor_preaprobado || !canAddQuote(petition.estado))) throw new Error('Esta petición ya no admite cotizaciones.')
          ...
          if (!current && await repo.countQuotes(query, petition.id) >= (preapproved ? 1 : MAX_COTIZACIONES)) {
            throw new Error(preapproved ? 'Esta petición admite una cotización del servicio.' : 'La petición admite hasta diez cotizaciones.')
          }

          const reason = String(preapproved ? petition.proveedor_preaprobado_nombre || '' : input.proveedorRazonSocial || '').trim()
          const country = preapproved ? null : String(input.proveedorPais || '').trim().toUpperCase()
          const fiscal = preapproved ? null : String(input.proveedorIdFiscal || '').trim()
          if (preapproved) {
            if (!normalizeSupplierName(reason) || reason.length > 200) throw new Error('Guarda el nombre del proveedor aprobado antes de adjuntar la cotización.')
          } else {
          ...
          const quote = await repo.prepareQuote(query, {
            ...
            proveedor_id_fiscal_clave: preapproved ? null : normalizeFiscalId(fiscal),
            proveedor_preaprobado: preapproved,
            empresa_constituida: preapproved ? null : true,
            emite_factura_fiscal: preapproved ? null : true,
    ...
    async authorizeToken(actor, input) {
      ...
      assertQuoteWriteAccess(actor, row)
      ...
      if (row.submitted_at && (row.proveedor_preaprobado || !canAddQuote(row.estado))) throw new Error('Esta petición ya no admite cotizaciones.')
      return {
    ...
    async complete({ blob: uploaded, tokenPayload }) {
      ...
      if (context.submitted_at && (context.proveedor_preaprobado || !canAddQuote(context.estado))) {
      ...
      try {
        const finalized = await repo.transaction(async (query) => {
          ...
          if (petition.submitted_at && (petition.proveedor_preaprobado || !canAddQuote(petition.estado))) return { terminal: true }
    ...
    async discardAttempt(actor, input) {
      return repo.transaction(async (query) => {
        ...
        assertWriteCentro(actor, petition.centro_id)
        if (!petition.submitted_at && String(petition.created_by) !== String(actor.id) && !isMaster(actor)) throw new Error('No autorizado')
        if (petition.submitted_at && (petition.proveedor_preaprobado || !canAddQuote(petition.estado))) throw new Error('Esta petición ya no admite cambios documentales.')
