...
for (const [actor, returnLabel] of [['admin', 'Volver a Administración'], ['coordinator', 'Volver al panel']]) {
  test(`${actor}: retorno persistente desde centro y gate Zoho`, async ({ browser }, testInfo) => {
    ...
    try {
      ...
      const recorrido = page.getByRole('link', { name: 'Iniciar recorrido' })
      await expect(recorrido).toBeVisible()
      await expect(recorrido).toHaveAttribute('href', `/centro/${process.env.E2E_CENTRO_ID}?tour=meta&paso=1`)
...
test('centro conserva CTA recomendado, dos pasos y regreso al índice sin completar', async ({ browser }, testInfo) => {
  ...
  try {
    ...
    await expect(page.getByRole('link', { name: /Siguiente módulo:/ })).toHaveCount(0)
    const volver = page.getByRole('link', { name: 'Volver a Entrenamiento' })
    await expect(volver).toHaveAttribute('href', training)
    await volver.focus()
