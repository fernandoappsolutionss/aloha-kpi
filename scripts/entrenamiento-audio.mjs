...
// clip: solo regenera lo que cambió de texto (o de voz/formato/settings). Se
// corre en la Mac; los mp3 se commitean en public/entrenamiento/.
...
function guardarManifest(manifest) {
...
async function main() {
  ...
  if (args.includes('--solo') && !MODULOS.some((m) => m.id === SOLO)) {
    console.error(`✗ --solo ${SOLO || '(sin valor)'}: ese módulo no existe. Válidos: ${MODULOS.map((m) => m.id).join(', ')}`)
    process.exit(1)
  }

  // La key se resuelve UNA vez, antes de tocar disco: si falta, falla ya.
  const KEY = apiKey()
